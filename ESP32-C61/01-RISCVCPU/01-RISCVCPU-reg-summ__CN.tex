...
\hyperref[regdesc:MINTTHRESH]{mintthresh} & 中断阈值（参见 CLIC 规范） & 0x347 & R/W \\\hline
...
\hyperref[regdesc:UTVEC]{utvec} \footnote{\hyperref[regdesc:UTVEC]{utvec} 保存了陷阱向量配置，包括向量基地址和向量模式} & 用户陷阱向量 & 0x005 & R/W \\\hline
\hyperref[regdesc:UTVT]{utvt} & 用户向量中断基地址 (参见 CLIC 规范) & 0x007 & R/W \\\hline
\multicolumn{4}{|l|}{\textbf{用户陷阱处理 CSRs}} \\\hline
\hyperref[regdesc:USCRATCH]{uscratch} & 用户暂存 & 0x040 & R/W \\\hline
\hyperref[regdesc:UEPC]{uepc} & 用户陷阱程序计数器 & 0x041 & R/W \\\hline
\hyperref[regdesc:UCAUSE]{ucause} \footnote{\hyperref[regdesc:UCAUSE]{ucause} 中反映的外部中断 ID 也包括 RISC-V 标准为核心内部中断源预留的 ID。} & 用户陷阱原因 & 0x042 & R/W \\\hline
\hyperref[regdesc:UNXTI_REG]{unxti} & 中断处理程序地址和使能 (参见 CLIC 规范) & 0x045 & R/W \\\hline
...
\hyperref[sec:riscvcpu-pmp-reg-desc]{pmpcfg0} & 物理存储器保护配置 & 0x3A0 & R/W \\\hline
