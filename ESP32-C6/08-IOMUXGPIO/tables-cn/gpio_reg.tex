...
\label{fielddesc:GPIOOUTDATAORIG}\item [GPIO\_OUT\_DATA\_ORIG] 配置简单 GPIO 输出模式下 GPIO0 \verb+~+ GPIO30 的输出值。 \\
0：低电平\\
1：高电平\\
bit0 \verb+~+ bit30 分别对应 GPIO0 \verb+~+ GPIO30 的输出值。bit31 无效。\\
(R/W/SC/WTC)
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:GPIOOUTW1TS}\item [GPIO\_OUT\_W1TS] 配置是否置位 GPIO0 \verb+~+ GPIO30 输出寄存器 \hyperref[regdesc:GPIOOUTREG]{GPIO\_OUT\_REG}。\\
0：不置位\\
1：\hyperref[regdesc:GPIOOUTREG]{GPIO\_OUT\_REG} 中相应位也将置 1\\
bit0 \verb+~+ bit30 分别对应 GPIO0 \verb+~+ GPIO30。bit31 无效。推荐使用此寄存器来置位 \hyperref[regdesc:GPIOOUTREG]{GPIO\_OUT\_REG}。\\
(WT)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:GPIOOUTW1TC}\item [GPIO\_OUT\_W1TC] 配置是否清除 GPIO0 \verb+~+ GPIO30 输出寄存器 \hyperref[regdesc:GPIOOUTREG]{GPIO\_OUT\_REG}。\\
0：不清除\\
1：\hyperref[regdesc:GPIOOUTREG]{GPIO\_OUT\_REG} 中相应位将被清除\\
bit0 \verb+~+ bit30 分别对应 GPIO0 \verb+~+ GPIO30。bit31 无效。推荐使用该寄存器来清除 \hyperref[regdesc:GPIOOUTREG]{GPIO\_OUT\_REG}。\\
(WT)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:GPIOENABLEDATA}\item [GPIO\_ENABLE\_DATA] 配置是否使能 GPIO0 \verb+~+ GPIO30 输出。\\
0：不使能\\
1：使能\\
bit0 \verb+~+ bit30 分别对应 GPIO0 \verb+~+ GPIO30。bit31 无效。\\
(R/W/WTC)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:GPIOENABLEW1TS}\item [GPIO\_ENABLE\_W1TS] 配置是否使能 GPIO0 \verb+~+ GPIO30 的输出使能寄存器 \hyperref[regdesc:GPIOENABLEREG]{GPIO\_ENABLE\_REG}。\\
0：不使能\\
1：\hyperref[regdesc:GPIOENABLEREG]{GPIO\_ENABLE\_REG} 中相应位也将置 1\\
bit0 \verb+~+ bit30 分别对应 GPIO0 \verb+~+ GPIO30。bit31 无效。推荐使用该寄存器来置位 \hyperref[regdesc:GPIOENABLEREG]{GPIO\_ENABLE\_REG}。\\
(WT)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:GPIOENABLEW1TC}\item [GPIO\_ENABLE\_W1TC] 配置是否清除 GPIO0 \verb+~+ GPIO30 输出使能寄存器 \hyperref[regdesc:GPIOENABLEREG]{GPIO\_ENABLE\_REG}。 \\
0：不清除\\
1：\hyperref[regdesc:GPIOENABLEREG]{GPIO\_ENABLE\_REG} 中相应位将被清除\\
bit0 \verb+~+ bit30 分别对应 GPIO0 \verb+~+ 30。bit31 无效。推荐使用此寄存器清除 \hyperref[regdesc:GPIOENABLEREG]{GPIO\_ENABLE\_REG}。\\
(WT)
\end{reglist}\end{regdesc}
\end{register}
...
\begin{itemize}
    \item bit0 \verb+~+ bit1：无效
    \item bit2：表示 GPIO8 管脚的值
    \item bit3：表示 GPIO9 管脚的值
    \item bit4：表示 GPIO15 管脚的值
    \item bit5：表示 MTMS 管脚的值
    \item bit6：表示 MTDI 管脚的值
    \item bit7 \verb+~+ bit15：无效
\end{itemize}
(RO)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:GPIOINDATANEXT}\item [GPIO\_IN\_DATA\_NEXT] 表示 GPIO0 \verb+~+ GPIO30 的输入值。每一位均代表一个管脚的输入值：\\
0：低电平 \\
1：高电平 \\
bit0 \verb+~+ bit30 分别对应 GPIO0 \verb+~+ GPIO30，bit31 无效。\\
(RO)
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:GPIOSTATUSINTERRUPT}\item [GPIO\_STATUS\_INTERRUPT] GPIO0 \verb+~+ GPIO30 的中断状态寄存器，并且软件可配置该寄存器的值。 
\begin{itemize}
    \item bit0 \verb+~+ bit30 分别对应 GPIO0 \verb+~+ GPIO30。bit31 无效。  
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:GPIOSTATUSW1TS}\item [GPIO\_STATUS\_W1TS] 配置是否置位 GPIO0 \verb+~+ GPIO30 的中断状态寄存器 \hyperref[fielddesc:GPIOSTATUSINTERRUPT]{GPIO\_STATUS\_INTERRUPT}。 
\begin{itemize}
    \item bit0 \verb+~+ bit30 分别对应 GPIO0 \verb+~+ GPIO30。bit31 无效。
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:GPIOSTATUSW1TC}\item [GPIO\_STATUS\_W1TC] 配置是否清除 GPIO0 \verb+~+ GPIO30 的中断状态寄存器 \hyperref[fielddesc:GPIOSTATUSINTERRUPT]{GPIO\_STATUS\_INTERRUPT}。
\begin{itemize}
    \item bit0 \verb+~+ bit30 分别对应 GPIO0 \verb+~+ GPIO30。bit31 无效。
    \item 每一位置 1，则 \hyperref[fielddesc:GPIOSTATUSINTERRUPT]{GPIO\_STATUS\_INTERRUPT} 中相应位将被清除。 
    \item 推荐使用此寄存器来清除 \hyperref[fielddesc:GPIOSTATUSINTERRUPT]{GPIO\_STATUS\_INTERRUPT}。 \end{itemize}(WT)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:GPIOPROCPUINT}\item [GPIO\_PROCPU\_INT] 表示 GPIO0 \verb+~+ GPIO30 CPU 中断状态。 
\begin{itemize}
    \item bit0 \verb+~+ bit30 分别对应 GPIO0 \verb+~+ GPIO30。bit31 无效。
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:GPIOSTATUSINTERRUPTNEXT}\item [GPIO\_STATUS\_INTERRUPT\_NEXT] 表示 GPIO0 \verb+~+ GPIO30 的中断源信号状态。\\
bit0 \verb+~+ bit30 分别对应 GPIO0 \verb+~+ 30。bit31 无效，每个 bit 表示：
\begin{itemize}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:GPIOFUNCNINSEL}\item [GPIO\_FUNC\regindex{n}\_IN\_SEL] 配置从 31 个 GPIO 中选择一个管脚连接输入信号 \regindex{n}。\\
0：选择 GPIO0\\
1：选择 GPIO1\\
......\\
29：选择 GPIO29\\
