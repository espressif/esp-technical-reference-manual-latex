...
\begin{TableNotes}
    ...
    \end{itemize}
    \item[2] Invalid combination: This combination triggers unexpected behavior and should be avoided.
\end{TableNotes}
...
\label{tab:bootctrl-boot-mode-control} \\
  ...
  \textbf{Boot Mode} & \textbf{GPIO9} & \textbf{GPIO8}\\
    ...
    \insertTableNotes
    \endlastfoot
  SPI Boot mode & 1 & Ignored \\ \hline
  Joint Download Boot mode\tnote{1} & 0 & 1\\ \hline
  Invalid combination\tnote{2} & 0 & 0 \\ \hline
\end{longtable}
\end{ThreePartTable}
...
\end{itemize}
In Joint Download Boot mode, users can download code into flash using UART0, USB Serial/JTAG Controller, or SDIO interface. It is also possible to load a program into SRAM and execute it from SRAM.
...
\begin{itemize}
    ...
    If this eFuse is 1, Joint Download Boot mode is permanently disabled. \hyperref[fielddesc:GPIOSTRAPPING]{GPIO\_STRAPPING} will not be overwritten by \hyperref[fielddesc:LPAONFORCEDOWNLOADBOOT]{LP\_AON\_FORCE\_DOWNLOAD\_BOOT}.
