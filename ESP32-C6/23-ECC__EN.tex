...
\subsection{Definitions of \chipname{}'s ECC}
...
\begin{table}[H]
    ...
    \end{tabular}
    \begin{tablenotes}
        \item[*] Address offset related to ECC accelerator base address is provided in Table \ref{tab:sysmem-base-address} in Chapter \ref{mod:sysmem} \textit{\nameref{mod:sysmem}}.
    \end{tablenotes}
    \end{threeparttable}
\end{table}
...
\chipname{}'s ECC operates on data of 256 bits. This data ($D[255:0]$) can be divided into eight 32-bit data blocks $D[n][31:0] (n=0, 1, \cdots, 7)$. Data blocks with the smaller serial number correspond to the lower binary bits. To be specific:
\[
...
Write data means writing data to an ECC memory block and using this data as the input to the ECC algorithm. To be specific, write data to an ECC memory block means writing $D[n][31:0] (n=0, 1, \cdots, 7)$ to the ``starting address of this ECC memory block + $
...
Read data means reading data from the starting address of an ECC memory block and using this data as the output from the ECC algorithm. To be specific, read data from an ECC memory block means reading $D[n][31:0] (n=0, 1, \cdots, 7)$ from the ``starting address of this ECC memory block + 
$4 \times n$" successively:
\begin{itemize}
...
\chipname{}'s ECC supports acceleration based on two key sizes (corresponding to two different elliptic curves). By configuring the \hyperref[fielddesc:ECCMULTKEYLENGTH]{ECC\_MULT\_KEY\_LENGTH} field, users can select the desired key size. For details, see Table \ref{tab:ecc-key-length} below.
...
\chipname{}'s ECC accelerator supports six working modes based on two elliptic curves described in the above section. By configuring the \hyperref[fielddesc:ECCMULTWORKMODE]{ECC\_MULT\_WORK\_MODE} field, users can choose the desired working mode. For details, see Table \ref{tab:ecc-work-mode}.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|L{2.8cm}|l|L{5.2cm}|}
    \hline
        \rowcolor{lightgray}
        ...
        3'd0 & Point Multi       & 3'd4 & Jacobian Point Multi             \\\hline
        3'd1 & \textbf{Reserved}         & 3'd5 & \textbf{Reserved}  \\\hline
        3'd2 & Point Verif   & 3'd6 & Jacobian Point Verif          \\\hline
        3'd3 & Point Verif + Multi  & 3'd7 & Point Verif + Jacobian Point Multi \\\hline
    \end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item Output: the verification result is stored in the \hyperref[fielddesc:ECCMULTVERIFICATIONRESULT]{ECC\_MULT\_VERIFICATION\_RESULT} field.
\end{itemize}
...
\begin{itemize}
    ...
    \item Output: 
    \begin{itemize}
        \item the verification result is stored in the \hyperref[fielddesc:ECCMULTVERIFICATIONRESULT]{ECC\_MULT\_VERIFICATION\_RESULT} field.
...
\chipname{}'s ECC only has one clock module (CRYPTO\_ECC\_CLK) and one reset module (CRYPTO\_ECC\_RST). Users should enable the ECC clock and disable the ECC reset before starting the ECC accelerator. For details on how to configure the ECC clock and reset, please refer to Chapter \ref{mod:resclk} \textit{\nameref{mod:resclk}}.
...
\begin{itemize}
    ...
    \item \hyperref[fielddesc:ECCMULTCALCDONEINTST]{ECC\_MULT\_CALC\_DONE\_INT\_ST}: indicates the status of the ECC\_MULT\_CALC\_DONE\_INT interrupt. This field is generated by enabling/disabling the \hyperref[fielddesc:ECCMULTCALCDONEINTRAW]{ECC\_MULT\_CALC\_DONE\_INT\_RAW} field via \hyperref[fielddesc:ECCMULTCALCDONEINTENA]{ECC\_MULT\_CALC\_DONE\_INT\_ENA}.
    ...
    \item \hyperref[fielddesc:ECCMULTCALCDONEINTCLR]{ECC\_MULT\_CALC\_DONE\_INT\_CLR}: set this bit to clear the ECC\_MULT\_CALC\_DONE\_INT interrupt status. By setting this bit to 1, fields \hyperref[fielddesc:ECCMULTCALCDONEINTRAW]{ECC\_MULT\_CALC\_DONE\_INT\_RAW} and  \hyperref[fielddesc:ECCMULTCALCDONEINTST]{ECC\_MULT\_CALC\_DONE\_INT\_ST} will be cleared.
\end{itemize}
...
\begin{enumerate}
    ...
    \item Choose the key size and working mode as described in Section \ref{sec:ecc-mode-config}. 
