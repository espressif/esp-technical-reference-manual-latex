...
\input{\modulefiles/tables/28-EXTMEMENCR-key-possibility__EN}
...
For example, when the \begin{math} size \end{math} is 32, the mapping between $o{\f}{\f}set$ and addresses in the plaintext memory region is shown in Table \ref{tab:extmemencr-data-padding}.
...
\begin{enumerate}
    ...
    \item Wait for Manual Encryption block to be idle. Poll register \hyperref[regdesc:SPIMEMXTSSTATEREG]{SPI\_MEM\_XTS\_STATE\_REG} until it reads 0 that indicates the Manual Encryption block is idle.
