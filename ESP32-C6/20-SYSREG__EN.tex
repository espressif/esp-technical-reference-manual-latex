...
\chipname{} supports a set of auxiliary chip features listed in subsection  \ref{sec:sysreg-features} \textit{\nameref{sec:sysreg-features}} below, which are configured via registers. This chapter provides a description of the registers used to configure these features.
...
\hyperref[regdesc:HPSYSTEMEXTERNALDEVICEENCRYPTDECRYPTCONTROLREG]{HP\_SYSTEM\_EXTERNAL\_DEVICE\_ENCRYPT\_DECRYPT\_CONTROL\_REG} configures encryption and decryption options of the external memory. For details, please refer to Chapter \ref{mod:extmemencr}
...
\subsection{HP Core/LP Core Debug Control}

The following register is used to debug between HP CPU and LP CPU. For more information on how to debug HP CPU and LP CPU, please refer to the Subsection \ref{sec:riscvcpu-dbg} \textit{\nameref{sec:riscvcpu-dbg}} in Chapter \ref{mod:riscvcpu} \textit{\nameref{mod:riscvcpu}}.

\begin{itemize}
    \item \hyperref[fielddesc:HPSYSTEMCOREDEBUGRUNSTALLENABLE]{HP\_SYSTEM\_CORE\_DEBUG\_RUNSTALL\_ENABLE}: Enable this bit to enable debug RunStall feature between HP CPU and LP CPU.
\end{itemize}
...
The addresses in this section are relative to System Registers base address provided in Table \ref{tab:sysmem-base-address} in Chapter \ref{mod:sysmem} \textit{\nameref{mod:sysmem}}.

The abbreviations given in Column \textbf{Access} are explained in Section \hyperref[glossary-access-types]{\textit{Access Types for Registers}}.

\subfile{\modulefiles/20-SYSREG-reg-summ__EN}
\end{landscape}
...
\subfile{\modulefiles/20-SYSREG-reg__EN}
