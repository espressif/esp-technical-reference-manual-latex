...
\begin{enumerate}
    ...
    \end{itemize}
    \item Configure the \hyperref[regdesc:SHAIRQENAREG]{SHA\_IRQ\_ENA\_REG} register to enable or disable interrupt (Set 1 to enable).
    ...
    \item Wait till the completion of the DMA-SHA calculation, which happens when:
        \begin{itemize}
            ...
            \item An SHA interrupt occurs. In this case, please clear interrupt by writing 1 to the \hyperref[regdesc:SHACLEARIRQREG]{SHA\_CLEAR\_IRQ\_REG} register.
        \end{itemize}
...
\begin{itemize}
    \item To enable this function: write 1 to register \hyperref[regdesc:SHAIRQENAREG]{SHA\_IRQ\_ENA\_REG}. 
    \item Note that the interrupt should be cleared by software after use via setting the \hyperref[regdesc:SHACLEARIRQREG]{SHA\_CLEAR\_IRQ\_REG} register to 1.
\end{itemize}  
