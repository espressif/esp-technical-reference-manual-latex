...
(R/W)
\label{fielddesc:HPSYSTEMSECDPACFGSEL}\item [HP\_SYSTEM\_SEC\_DPA\_CFG\_SEL] 配置选择 HP\_SYSTEM\_SEC\_DPA\_LEVEL 或是 EFUSE\_SEC\_DPA\_LEVEL (eFuse 寄存器) 来控制 DPA 等级。\\
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:HPSYSTEMCOREDEBUGRUNSTALLENABLE}\item [HP\_SYSTEM\_CORE\_DEBUG\_RUNSTALL\_ENABLE] 配置是否在 HP CPU 和 LP CPU 之间启用调试 RunStall 功能。\\
0：关闭\\
1：开启\\
(R/W)
\end{reglist}\end{regdesc}
...
\label{fielddesc:HPSYSTEMCPUPERITIMEOUTINTCLEAR}\item [HP\_SYSTEM\_CPU\_PERI\_TIMEOUT\_INT\_CLEAR] 配置是否清除超时中断。\\
0：无效\\
1：清除超时中断\\
(WT)
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:HPSYSTEMCPUPERITIMEOUTUID}\item [HP\_SYSTEM\_CPU\_PERI\_TIMEOUT\_UID] 表示超时发生时的主站 ID [4:0] 和主站权限 [6:5]。该寄存器将在清除超时中断后清除。 (WTC)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:HPSYSTEMHPPERITIMEOUTUID}\item [HP\_SYSTEM\_HP\_PERI\_TIMEOUT\_UID] 表示超时发生时的主站 ID [4:0] 和主站权限 [6:5]。该寄存器将在清除超时中断后清除。 (WTC)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:LPPERIBUSTIMEOUTUID}\item [LP\_PERI\_BUS\_TIMEOUT\_UID] 表示超时发生时的主站 ID [4:0] 和主站权限 [6:5]。该寄存器将在清除超时中断后清除。 (WTC)
\end{reglist}\end{regdesc}
\end{register}
