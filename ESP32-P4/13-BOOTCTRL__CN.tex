...
\begin{itemize}
    ...
    如果此 eFuse 设置为 0（默认），软件可通过设置 \hyperref[fielddesc:LPSYSTEMFORCEDOWNLOADBOOT]{LP\_SYSTEM\_FORCE\_DOWNLOAD\_BOOT}，触发 CPU 复位，将芯片 Boot 模式强制从 SPI Boot 模式切换至 Joint Download 模式；如果此 eFuse 设置为 1，则禁用 \hyperref[fielddesc:LPSYSTEMFORCEDOWNLOADBOOT]{LP\_SYSTEM\_FORCE\_DOWNLOAD\_BOOT}。
...
\hyperref[fielddesc:EFUSEDISUSBSERIALJTAGROMPRINT]{EFUSE\_DIS\_USB\_SERIAL\_JTAG\_ROM\_PRINT} 控制打印至 \textbf{USB 串口/JTAG 控制器}。该位为 1 时禁用打印至 USB 串口/JTAG 控制器。该位为 0 时，ROM 日志可打印到 USB 串口/JTAG 控制器。
