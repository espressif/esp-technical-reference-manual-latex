...
为了更好地说明 GP-SPI 和 LP-SPI 的功能，本章使用了以下术语。 

\begin{longtable}[c]{ R{4cm} p{10cm} }

\textbf{主机模式}     & GP-SPI 或 LP-SPI 用作 SPI 主机，发起 SPI 传输事务。\\ 
\textbf{从机模式}      & GP-SPI 或 LP-SPI 用作 SPI 从机，当其 CS 被拉低时，与 SPI 主机进行数据传输。\\
...
\textbf{CPU 控制的传输}& 由 CPU 控制，\hyperref[regdesc:SPIW0REG]{SPI\_W0\_REG}$\sim$\hyperref[regdesc:SPIW15REG]{SPI\_W15\_REG}（或 \hyperref[regdesc:LPSPIW0REG]{LP\_SPI\_W0\_REG}$\sim$\hyperref[regdesc:LPSPIW15REG]{LP\_SPI\_W15\_REG}）与 SPI 设备之间的数据传输。\\
\textbf{DMA 控制的传输}& 由 DMA 引擎控制，DMA 与 SPI 设备之间的数据传输。\\
\textbf{分段配置传输}& GP-SPI2 用作主机时，DMA 控制的数据传输。此类传输包含多个传输事务（分段），每个传输事务均可独立配置。\\
\textbf{从机连续传输} & GP-SPI 用作从机时，DMA 控制的数据传输。此类传输包含多个传输事务（分段）。\\
...
\textbf{(8-bit) Octal SPI} & 一个时钟周期传输八个数据位。 \\
\textbf{Octal Output Read} & Octal SPI 的一种数据模式，一个时钟周期可传输一位命令、或一位地址、或八位数据。\\
\textbf{Octal I/O Read} & Octal SPI 的另外一种数据模式，一个时钟周期可传输一位命令、或八位地址、或八位数据。\\
\textbf{OPI} & 一个时钟周期可传输八位命令、或八位地址、或八位数据。\\
\textbf{SPI2} & GP-SPI2 输入输出信号的前缀。SPI2 总线信号可通过 HP GPIO 交换矩阵或 HP IO MUX 与 GPIO 管脚相连。\\
\textbf{SPI3} & GP-SPI3 输入输出信号的前缀。SPI3 总线信号仅可通过 HP GPIO 交换矩阵连接到 GPIO 管脚。\\
\textbf{LP\_SPI} & LP-SPI 输入输出信号的前缀。LP\_SPI 总线信号仅可通过 LP GPIO 交换矩阵连接到 GPIO 管脚。
\end{longtable}
