...
\chipname{} has three I2C controllers: two in the main system and one in the low-power system. The two I2C controllers in the main system can act as a master or a slave (referred to as I2C below), while the one in the low-power system can only act as a master (referred to as LP\_I2C below), which can still work when the main system sleeps.
...
\begin{itemize}
    ...
    \item SCL clock controller (SCL\_FSM): generate the timing sequence conforming to the I2C protocol. Figure \ref{fig:i2c-protocol-timing} and Table \ref{tab:i2c-timing-param} are the timing diagram and corresponding parameters of the I2C protocol.
...
You can configure the clock source for I2C\_SCLK of I2C0 in the main system (HP) to FOSC\_20M\_CLK or XTAL\_40M\_CLK via \hyperref[fielddesc:HPSYSCLKRSTI2C0CLKSRCSEL]{HP\_SYS\_CLKRST\_I2C0\_CLK\_SRC\_SEL} and configure that of I2C1 in the main system via \hyperref[fielddesc:HPSYSCLKRSTI2C1CLKSRCSEL]{HP\_SYS\_CLKRST\_I2C1\_CLK\_SRC\_SEL}. For the clock source for I2C\_SCLK of LP\_I2C in the low-power system (LP), you can configure it to XTAL\_D2\_CLK, LP\_DYN\_FAST\_CLK, or PLL\_LP\_CLK 8 MHz via \hyperref[fielddesc:LPPERILPI2CCLKSEL]{LPPERI\_LP\_I2C\_CLK\_SEL}.

The steps to configure the clock source for HP I2C0 are as follows:
\begin{itemize}
    \item Enable the clock source for I2C\_SCLK of I2C0 by configuring \hyperref[fielddesc:HPSYSCLKRSTI2C0CLKEN]{HP\_SYS\_CLKRST\_I2C0\_CLK\_EN} to 1.
    \item When \hyperref[fielddesc:HPSYSCLKRSTI2C0CLKSRCSEL]{HP\_SYS\_CLKRST\_I2C0\_CLK\_SRC\_SEL} is 0, the clock source is XTAL\_40M\_CLK.
    \item When \hyperref[fielddesc:HPSYSCLKRSTI2C0CLKSRCSEL]{HP\_SYS\_CLKRST\_I2C0\_CLK\_SRC\_SEL} is 1, the clock source is FOSC\_20M\_CLK.
\end{itemize}

The steps to configure the clock source for HP I2C1 are as follows:
\begin{itemize}
    \item Enable the clock source for I2C\_SCLK of I2C1 by configuring \hyperref[fielddesc:HPSYSCLKRSTI2C1CLKEN]{HP\_SYS\_CLKRST\_I2C1\_CLK\_EN} to 1.
    \item When \hyperref[fielddesc:HPSYSCLKRSTI2C1CLKSRCSEL]{HP\_SYS\_CLKRST\_I2C1\_CLK\_SRC\_SEL} is 0, the clock source is XTAL\_40M\_CLK.
    \item When \hyperref[fielddesc:HPSYSCLKRSTI2C1CLKSRCSEL]{HP\_SYS\_CLKRST\_I2C1\_CLK\_SRC\_SEL} is 1, the clock source is FOSC\_20M\_CLK.
\end{itemize}

The steps to configure the clock source for LP\_I2C are as follows:
\begin{itemize}
    \item Enable the clock source for I2C\_SCLK of LP\_I2C by configuring \hyperref[fielddesc:LPPERICKENLPI2C]{LPPERI\_CK\_EN\_LP\_I2C} to 1.
    \item When \hyperref[fielddesc:LPPERILPI2CCLKSEL]{LPPERI\_LP\_I2C\_CLK\_SEL} is 0, the clock source is LP\_DYN\_FAST\_CLK.
    \item When \hyperref[fielddesc:LPPERILPI2CCLKSEL]{LPPERI\_LP\_I2C\_CLK\_SEL} is 1, the clock source is XTAL\_D2\_CLK.
    \item When \hyperref[fielddesc:LPPERILPI2CCLKSEL]{LPPERI\_LP\_I2C\_CLK\_SEL} is 2, the clock source is PLL\_LP\_CLK 8 MHz.
\end{itemize}
...
In the equation, I2C\_SCLK\_DIV\_NUM represents the integer part of the divisor, I2C\_SCLK\_DIV\_A represents the numerator of the fractional part of the divisor, and I2C\_SCLK\_DIV\_B represents the denominator of the fractional part of the divisor. Limited by timing parameters, the derived clock I2C\_SCLK should operate at a frequency 20 times larger than SCL's frequency.\\

For I2C0:
\begin{itemize}
    \item Configure I2C\_SCLK\_DIV\_NUM via \hyperref[fielddesc:HPSYSCLKRSTI2C0CLKDIVNUM]{HP\_SYS\_CLKRST\_I2C0\_CLK\_DIV\_NUM}.
    \item Configure I2C\_SCLK\_DIV\_A via \hyperref[fielddesc:HPSYSCLKRSTI2C0CLKDIVNUMERATOR]{HP\_SYS\_CLKRST\_I2C0\_CLK\_DIV\_NUMERATOR}.
    \item Configure I2C\_SCLK\_DIV\_B via \hyperref[fielddesc:HPSYSCLKRSTI2C0CLKDIVDENOMINATOR]{HP\_SYS\_CLKRST\_I2C0\_CLK\_DIV\_DENOMINATOR}.
\end{itemize}

For I2C1:
\begin{itemize}
    \item Configure I2C\_SCLK\_DIV\_NUM via \hyperref[fielddesc:HPSYSCLKRSTI2C1CLKDIVNUM]{HP\_SYS\_CLKRST\_I2C1\_CLK\_DIV\_NUM}.
    \item Configure I2C\_SCLK\_DIV\_A via \hyperref[fielddesc:HPSYSCLKRSTI2C1CLKDIVNUMERATOR]{HP\_SYS\_CLKRST\_I2C1\_CLK\_DIV\_NUMERATOR}.
    \item Configure I2C\_SCLK\_DIV\_B via \hyperref[fielddesc:HPSYSCLKRSTI2C1CLKDIVDENOMINATOR]{HP\_SYS\_CLKRST\_I2C1\_CLK\_DIV\_DENOMINATOR}.
\end{itemize}
...
\caption{I2C Synchronous Registers}\label{tab:i2c-sync-register}\\\hline
\rowcolor{lightgray}
...
\subsection{To Start the I2C Controller}
...
\item The clock source of APB\_CLK in LP\_I2C is CLK\_AON\_FAST. The clock source for I2C\_SCLK is configured via \hyperref[fielddesc:LPPERILPI2CCLKSEL]{LPPERI\_LP\_I2C\_CLK\_SEL}. The corresponding bits are:
    \begin{itemize}
        \item 0: CLK\_ROOT\_FAST
        \item 1: CLK\_XTALD2
        \item 2: PLL\_F8M\_CLK
    \end{itemize}
    Configuring \hyperref[fielddesc:LPPERILPI2CCLKSEL]{LPPERI\_CK\_EN\_LP\_I2C} to 1 enables the clock source of I2C\_SCLK. Adjust the timing registers accordingly when the clock frequency changes.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{\modulefiles/figures/45-I2C-MWS-END}    \caption{I2C$_\text{master}$ Writing to I2C$_\text{slave}$ with a 7-bit Address in Multiple Sequences}
...
\subfile{\modulefiles/45-I2C-ext-reg-summ__EN.tex}

\subsection{LP\_I2C Register Summary}
\label{sec:lp-i2c-reg-summ}
\hypertarget{lp-i2c-reg-summ}{}

The abbreviations given in Column \textbf{Access} are explained in Section \hyperref[glossary-access-types]{\textit{Access Types for Registers}}.

\subfile{\modulefiles/45-I2C-LP-ext-reg-summ__EN.tex}
...
\subfile{\modulefiles/45-I2C-ext-reg__EN.tex}

\subsection{LP\_I2C Registers}

\subfile{\modulefiles/45-I2C-LP-ext-reg__EN.tex}
