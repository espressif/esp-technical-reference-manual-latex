...
\begin{regdesc}
\begin{reglist}
\item [INT\_ENABLE\lbrack\regindex{n}\rbrack] 设置 \regindex{n} 位以使能 \regindex{n} 外部中断对 CPU 的断言。
0: 禁用\\
1: 启用\\
(R/W)
\end{reglist}
...
\begin{regdesc}
\begin{reglist}
\item [INT\_TYPE\lbrack\regindex{n}\rbrack] 设置 \regindex{n} 位以捕获 \regindex{n} 外部中断的上升沿。\\
0: 电平类型（信号电平检测）\\
1: 脉冲类型（上升沿检测）\\
(R/W)
\end{reglist}
...
\begin{regdesc}
\begin{reglist}
\item [INT\_CLEAR\lbrack\regindex{n}\rbrack] 设置 \regindex{n} 位以清除 \regindex{n} 外部中断的挂起状态。\\
0：无关紧要\\
1：清除挂起状态\\
这仅对“脉冲”类型中断有用，因为“电平”类型中断必须在源头清除。\\
注意，设置的位必须手动切换回 0。\\
(R/W)
\end{reglist}
\end{regdesc}
...
\begin{regdesc}
\begin{reglist}
\item [INT\_EIP\lbrack\regindex{n}\rbrack] 读取 \regindex{n} 位以获取 \regindex{n} 外部中断对 CPU 的挂起状态。\\
0：未挂起\\
1：挂起\\
只有启用且超过阈值的中断才会在此反映。\\
(RO)
...
\begin{regdesc}
\begin{reglist}
\label{fielddesc:INTTHRESH}\item [INT\_THRESH] 写入一个 4 位值以配置所有外部中断的全局优先级阈值。\\
所有优先级低于阈值的中断将被屏蔽。 \\
(R/W)
\end{reglist}
\end{regdesc}
