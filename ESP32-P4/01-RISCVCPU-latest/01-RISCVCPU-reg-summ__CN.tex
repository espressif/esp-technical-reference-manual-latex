...
\hyperref[regdesc:MINTSTATUS]{mintstatus} & 当前中断级别（参见 CLIC 规范） & 0xFB1 & RO \\\hline
\hyperref[regdesc:MINTTHRESH]{mintthresh} & 中断阈值（参见CLIC 规范）& 0x347 & R/W \\\hline
...
\multicolumn{4}{|l|}{\textbf{用户陷阱设置 CSR}} \\\hline
\hyperref[regdesc:USTATUS]{ustatus} & 用户模式状态 & 0x000 & R/W \\\hline
\hyperref[regdesc:UTVEC]{utvec} \footnote{\hyperref[regdesc:UTVEC]{utvec} 保存陷阱向量配置信息，包括向量基地址和向量模式。} & 用户陷阱向量 & 0x005 & R/W \\\hline
\hyperref[regdesc:UTVT]{utvt} & 用户向量中断基地址（参见 CLIC 规范） & 0x007 & R/W \\\hline
\multicolumn{4}{|l|}{\textbf{用户陷阱处理 CSR}} \\\hline
\hyperref[regdesc:USCRATCH]{uscratch} & 用户暂存寄存器 & 0x040 & R/W \\\hline
\hyperref[regdesc:UEPC]{uepc} & 用户陷阱程序计数器 & 0x041 & R/W \\\hline
\hyperref[regdesc:UCAUSE]{ucause} \footnote{\hyperref[regdesc:MCAUSE]{mcause} 中反映的外部中断 ID 也包括 RISC-V 标准为核心内部中断源预留的 ID。} & 用户陷阱原因 & 0x042 & R/W \\\hline
\hyperref[regdesc:UNXTI_REG]{unxti} & 中断处理程序地址和使能寄存器（参见 CLIC 规范） & 0x045 & R/W \\\hline
\hyperref[regdesc:UINTTHRESH]{uintthresh} & 中断阈值（参见 CLIC 规范） & 0x047 & R/W \\\hline
\hyperref[regdesc:UCLICBASE_REG]{uclicbase} & 用户模式中断控制器基地址寄存器（自定义）（参见 CLIC 规范） & 0x050 & RO \\\hline
\hyperref[regdesc:UINTSTATUS]{uintstatus} & 当前中断级别（参见 CLIC 规范） & 0xCB1 & RO \\\hline
...
\hyperref[sec:riscvcpu-pmp-reg-desc]{pmpcfg4} & 物理存储器保护配置 & 0x3A4 & R/W \\\hline
\hyperref[sec:riscvcpu-pmp-reg-desc]{pmpcfg5} & 物理存储器保护配置 & 0x3A5 & R/W \\\hline
\hyperref[sec:riscvcpu-pmp-reg-desc]{pmpcfg6} & 物理存储器保护配置 & 0x3A6 & R/W \\\hline
\hyperref[sec:riscvcpu-pmp-reg-desc]{pmpcfg7} & 物理存储器保护配置 & 0x3A7 & R/W \\\hline
\hyperref[sec:riscvcpu-pmp-reg-desc]{pmpaddr\regindex{n} (\regindex{n}: 0-31)} & 物理存储器保护地址寄存器 & {0x3B0+0x1*\regindex{n}} & R/W \\\hline
...
\hyperref[sec:riscvcpu-hwloop-reg-desc]{uhwloop\_state\_reg} & 用户硬件循环状态 & 0x8C6 & R/W \\\hline
