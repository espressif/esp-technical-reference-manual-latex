...
\label{fielddesc:I2CSCLLOWPERIOD}\item [I2C\_SCL\_LOW\_PERIOD] 用于配置 SCL 低电平的保持时间，以 I2C 主机控制器时钟周期数为单位。 (R/W)
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:I2CSDAHOLDTIME}\item [I2C\_SDA\_HOLD\_TIME] 用于配置 SCL 下降沿后的数据保持时间，以 I2C 主机控制器时钟周期数为单位。 (R/W)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:I2CSDASAMPLETIME}\item [I2C\_SDA\_SAMPLE\_TIME] 用于配置采样 SDA 的时间，以 I2C 主机控制器时钟周期数为单位。 (R/W)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:I2CSCLHIGHPERIOD}\item [I2C\_SCL\_HIGH\_PERIOD] 用于配置 SCL 保持高电平的时间，以 I2C 主机控制器时钟周期数为单位。 (R/W)
\label{fielddesc:I2CSCLWAITHIGHPERIOD}\item [I2C\_SCL\_WAIT\_HIGH\_PERIOD] 用于配置 SCL\_FSM 等待 SCL 翻转至高电平的时间，以 I2C 主机控制器时钟周期数为单位。 (R/W)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:I2CSCLSTARTHOLDTIME}\item [I2C\_SCL\_START\_HOLD\_TIME] 配置 START 命令产生时 SDA 下降沿 和 SCL 下降沿的间隔时间，以 I2C 主机控制器时钟周期数为单位。 (R/W)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:I2CSCLRSTARTSETUPTIME}\item [I2C\_SCL\_RSTART\_SETUP\_TIME] 配置 RSTART 命令产生时 SCL 上升沿和 SDA 下降沿的间隔时间，以 I2C 主机控制器的时钟周期数为单位。 (R/W)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:I2CSCLSTOPHOLDTIME}\item [I2C\_SCL\_STOP\_HOLD\_TIME] 配置 STOP 命令后的延迟，以 I2C 主机控制器时钟周期数为单位。 (R/W)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:I2CSCLSTOPSETUPTIME}\item [I2C\_SCL\_STOP\_SETUP\_TIME] 配置 SCL 上升沿和 SDA 上升沿的间隔时间，以 I2C 主机控制器时钟周期数为单位。 (R/W)
\end{reglist}\end{regdesc}
\end{register}
...
\label{fielddesc:I2CMSMODE}\item [I2C\_MS\_MODE] 置位此位，将 I2C 主机控制器配置为主机。清零此位，则 I2C 主机控制器不可运行。
 (R/W)
...
0：从最高有效位开始接收数据；
1：从最低有效位开始接收数据。 (R/W)
\label{fielddesc:I2CCLKEN}\item [I2C\_CLK\_EN] 保留。 (R/W)
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:I2CRXFIFOWMTHRHD}\item [I2C\_RXFIFO\_WM\_THRHD] 直接访问模式下，RX FIFO 的水标阈值。I2C\_FIFO\_PRT\_EN 为 1 且 RX FIFO 计数值大于 I2C\_TXFIFO\_WM\_THRHD[3:0] 时，I2C\_TXFIFO\_WM\_INT\_RAW 位有效。 (R/W)
\label{fielddesc:I2CTXFIFOWMTHRHD}\item [I2C\_TXFIFO\_WM\_THRHD] 直接访问模式下，TX FIFO 的水标阈值。I2C\_FIFO\_PRT\_EN 为 1 且 TX FIFO 计数值小于 I2C\_TXFIFO\_WM\_THRHD[3:0] 时，I2C\_TXFIFO\_WM\_INT\_RAW 位有效。 (R/W)
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:I2CSCLFILTERTHRES}\item [I2C\_SCL\_FILTER\_THRES] SCL 输入信号的脉冲宽度小于该字段的值时，I2C 主机控制器忽略此脉冲。该寄存器的值以 I2C 主机控制器时钟周期数为单位。 (R/W)
\label{fielddesc:I2CSDAFILTERTHRES}\item [I2C\_SDA\_FILTER\_THRES] SDA 输入信号的脉冲宽度小于该字段的值时，I2C 主机控制器忽略此脉冲。该寄存器的值以 I2C 主机控制器时钟周期数为单位。 (R/W)
...
\label{fielddesc:I2CSCLKSEL}\item [I2C\_SCLK\_SEL] 选择 I2C 主机控制器的时钟源。0：XTAL\_CLK；1：RC\_FAST\_CLK。 (R/W)
\label{fielddesc:I2CSCLKACTIVE}\item [I2C\_SCLK\_ACTIVE] I2C 主机控制器的时钟开关。 (R/W)
\end{reglist}\end{regdesc}
\end{register}
...
\label{fielddesc:I2CSCLRSTSLVNUM}\item [I2C\_SCL\_RST\_SLV\_NUM] 配置生成的 SCL 脉冲。I2C\_SCL\_RST\_SLV\_EN 为 1 时有效。 (R/W)
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:I2CRESPREC}\item [I2C\_RESP\_REC] 接收的 ACK 电平值。 0：ACK；1：NACK。 (RO)
\label{fielddesc:I2CARBLOST}\item [I2C\_ARB\_LOST] I2C 主机控制器不控制 SCL 线时，该寄存器变为 1。 (RO)
...
\label{fielddesc:I2CSCLMAINSTATELAST}\item [I2C\_SCL\_MAIN\_STATE\_LAST] 该字段为 I2C 主机控制器状态机的状态。
...
\label{fielddesc:I2CRXFIFOWADDR}\item [I2C\_RXFIFO\_WADDR] I2C 主机控制器接收数据和写 RX FIFO 的偏移地址。 (RO)
\label{fielddesc:I2CTXFIFORADDR}\item [I2C\_TXFIFO\_RADDR] I2C 主机控制器读 TX FIFO 的偏移地址。 (RO)
...
\end{itemize} (R/W)
\label{fielddesc:I2CCOMMAND0DONE}\item [I2C\_COMMAND0\_DONE] 完成命令 0 时，该位翻转为高电平。 (R/W/SS)
\end{reglist}\end{regdesc}
\end{register}
...
\label{fielddesc:I2CCOMMAND1DONE}\item [I2C\_COMMAND1\_DONE] 完成命令 1 时，该位翻转为高电平。 (R/W/SS)
\end{reglist}\end{regdesc}
\end{register}
...
\label{fielddesc:I2CCOMMAND2DONE}\item [I2C\_COMMAND2\_DONE] 完成命令 2 时，该位翻转为高电平。 (R/W/SS)
\end{reglist}\end{regdesc}
\end{register}
...
\label{fielddesc:I2CCOMMAND3DONE}\item [I2C\_COMMAND3\_DONE] 完成命令 3 时，该位翻转为高电平。 (R/W/SS)
\end{reglist}\end{regdesc}
\end{register}
...
\label{fielddesc:I2CCOMMAND4DONE}\item [I2C\_COMMAND4\_DONE] 完成命令 4 时，该位翻转为高电平。 (R/W/SS)
\end{reglist}\end{regdesc}
\end{register}
...
\label{fielddesc:I2CCOMMAND5DONE}\item [I2C\_COMMAND5\_DONE] 完成命令 5 时，该位翻转为高电平。 (R/W/SS)
\end{reglist}\end{regdesc}
\end{register}
...
\label{fielddesc:I2CCOMMAND6DONE}\item [I2C\_COMMAND6\_DONE] 完成命令 6 时，该位翻转为高电平。 (R/W/SS)
\end{reglist}\end{regdesc}
\end{register}
...
\label{fielddesc:I2CCOMMAND7DONE}\item [I2C\_COMMAND7\_DONE] 完成命令 7 时，该位翻转为高电平。 (R/W/SS)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:I2CTXFIFOSTARTADDR}\item [I2C\_TXFIFO\_START\_ADDR] I2C TX FIFO 的起始地址。 (HRO)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:I2CRXFIFOSTARTADDR}\item [I2C\_RXFIFO\_START\_ADDR] I2C RX FIFO 的起始地址。 (HRO)
\end{reglist}\end{regdesc}
\end{register}
