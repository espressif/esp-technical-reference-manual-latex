...
\begin{itemize}
    \item 支持 2 种可选 ECC 曲线，包括 \href{https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.186-5.pdf}{FIPS 186-5} 中定义的 P-192 和 P-256。
    \item 提供 7 种可选工作模式。
    \item 提供计算完成的中断和中断控制。
\end{itemize}
...
在 ECC 硬件加速器模块中会用到的数据位宽均为 256 位，假设一个数据为 $D[255:0]$，则其可以被划分为 8 个 32 bit 位宽的数据块 $D[n][31:0](n=0,1,\cdots,7)$，序号数低的数据块对应二进制低位，即：
\[
...
数据存储即将一个数据存储进一个内存块的操作，也可以说该数据为输入数据。具体来说，将数据写入一个 ECC 内存块相当于将该数据 $D[n][31:0] (n=0, 1, \cdots, 7)$ 依次写入“该内存块起始地址 + $
4 \times n$”：
...
数据读取即将一个数据从一个内存块读出的操作，也可以说该数据为输出数据。具体来说，从一个 ECC 内存块读数据相当于从“该内存块起始地址 + $
...
\chipname{} ECC 硬件加速器中，所有标准运算（包括标准点验证和标准点乘）的输入数据以及输出数据中的点均在仿射坐标系中；相对应的，所有 Jacobian 运算（包括 Jacobian 点验证和 Jacobian 点乘）的输入数据以及输出数据中的点均在 Jacobian 坐标系中。

\section{功能描述}
\label{sec:ecc-mode-config}
\subsection{密钥长度模式}

\chipname{} ECC 硬件加速器共支持 2 种密钥长度模式，每种密钥长度模式唯一对应一条椭圆曲线。用户通过配置寄存器  \hyperref[fielddesc:ECCMULTKEYLENGTH]{ECC\_MULT\_KEY\_LENGTH} 来选定密钥长度模式，其与椭圆曲线的对应关系如表 \ref{tab:ecc-key-length}。
\begin{table}[H]
    \centering
    ...
    \hline
        \rowcolor{lightgray}
        \textbf{\hyperref[fielddesc:ECCMULTKEYLENGTH]{ECC\_MULT\_KEY\_LENGTH}} & \textbf{对应椭圆曲线} \\ \hline
        1'b0 & FIPS P-192  \\\hline
        1'b1 & FIPS P-256  \\\hline
    \end{tabular}
    \begin{tablenotes}
        \item[1] FIPS P-192/P-256 的曲线定义在 \href{https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.186-5.pdf}{FIPS 186-5} 中描述。
...
\chipname{} ECC 硬件加速器共有 7 种工作模式，每种工作模式进行基于选定曲线的不同操作。用户通过配置寄存器 \hyperref[fielddesc:ECCMULTWORKMODE]{ECC\_MULT\_WORK\_MODE} 来选定工作模式，其与工作模式的对应关系如表\ref{tab:ecc-work-mode}。
...
\begin{table}[H]
    ...
    \hline
        ...
        3'd1 & 有限域除法          & 3'd5 & \textbf{保留项，不可用}  \\\hline
...
其中，
\begin{itemize}
    \item 输入数据：$P_y$，$k$对应的内存块为 \hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_Py} 和 \hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_k}。
    \item 输出数据：Result 对应的内存块为 \hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_Py}。
\end{itemize}
...
\begin{itemize}
    \item 输入数据：$P_x$，$P_y$，$k$对应的内存块为 \hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_Px}, \hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_Py} 和 \hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_k}。
    \item 输出数据：点验证的结果存储在寄存器 \hyperref[fielddesc:ECCMULTVERIFICATIONRESULT]{ECC\_MULT\_VERIFICATION\_RESULT} 中；$Q_x$、$Q_y$对应的内存块为 \hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_Px} 和 \hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_Py}。
\end{itemize}
\clearpage
...
\begin{itemize}
    ...
    \item 输入数据：$P_x$，$P_y$ 和 $k$对应的内存块为 \hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_Px}, \hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_Py}, \hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_k}。
    \item 输出数据：$Q_x$、$Q_y$ 和 $Q_z$对应的内存块为 \hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_Px}, \hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_Py}, \hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_k}。
\end{itemize}
...
\begin{itemize}
    ...
    \item 输入数据：$P_x$，$P_y$ 和 $k$对应的内存块为 \hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_Px}, \hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_Py}, \hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_k}。
    \item 输出数据：点验证的结果存储在寄存器 \hyperref[fielddesc:ECCMULTVERIFICATIONRESULT]{ECC\_MULT\_VERIFICATION\_RESULT} 中；$Q_x$、$Q_y$ 和 $Q_z$对应的内存块为 \hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_Px}，\hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_Py}，\hyperref[tab:ecc-memory]{ECC\_MULT\_Mem\_k}。
\end{itemize}

\section{时钟与复位}
\chipname{} ECC 硬件加速器模块仅有一个模块时钟 crypo\_ecc\_clk 和一个模块复位 crypto\_ecc\_rst 。在使用 ECC硬件加速器之前，需要开启 ECC 时钟，关闭 ECC 复位。如何配置 ECC 时钟和复位，请参考章节 \ref{mod:resclk} \textit{\nameref{mod:resclk}}。

\clearpage
\section{中断}
\label{sec:ecc-interrupt}

\chipname{} ECC 硬件加速器共可产生一个中断信号 ECC\_INTR，并将其发送给\hyperref[mod:intmtrx]{中断矩阵}。
...
\begin{itemize}
    \item \label{int:ECCMULTCALCDONEINT}ECC\_MULT\_CALC\_DONE\_INT：ECC 硬件加速器运算完成即触发该中断。
\end{itemize}

中断 ECC\_MULT\_CALC\_DONE\_INT 由以下寄存器控制：
\begin{itemize}
    \item \hyperref[fielddesc:ECCMULTCALCDONEINTRAW]{ECC\_MULT\_CALC\_DONE\_INT\_RAW}：ECC 硬件加速器运算完成时置1。
    ...
    \item \hyperref[fielddesc:ECCMULTCALCDONEINTCLR]{ECC\_MULT\_CALC\_DONE\_INT\_CLR}：置1此位清除 ECC 硬件加速器运算完成中断，对应的 \hyperref[fielddesc:ECCMULTCALCDONEINTRAW]{ECC\_MULT\_CALC\_DONE\_INT\_RAW} 和 \hyperref[fielddesc:ECCMULTCALCDONEINTST]{ECC\_MULT\_CALC\_DONE\_INT\_ST} 位会清零。
\end{itemize}
...
本小节的所有地址均为相对于  ECC 硬件加速器 基地址的地址偏移量（相对地址），具体基地址请见章节 \ref{mod:sysmem} \textit{\nameref{mod:sysmem}} 中的表 \ref{tab:sysmem-base-address}。

请查看章节 \hyperref[glossary-access-types]{\textit{寄存器的访问类型}}，了解“访问”列缩写的含义。

\subfile{\modulefiles/16-ecc-reg-summ__CN}
...
\subfile{\modulefiles/16-ecc-reg__CN}
