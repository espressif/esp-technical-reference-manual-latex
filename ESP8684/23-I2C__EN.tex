...
\hypertarget{i2c}{}
\chapter{I2C Master Controller (I2C)}
\label{mod:i2c}
...
\chipname{} provides one I2C controller operating in master mode.
...
\begin{itemize}
    ...
    \item Communication between multiple masters
    \item Standard mode (100 Kbit/s)
    \item Fast mode (400 Kbit/s)
    \item 7-bit and 10-bit slave addressing
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{\modulefiles/figures/729_i2c_master_architecture}
...
Figure \ref{fig:i2c-master} shows the architecture of an I2C master. The I2C master controller has the following main parts:
\begin{itemize}
    ...
    \item ACK bit controller (ACK\_deal)
\end{itemize}
...
Figure \ref{fig:i2c-protocol-timing} and Figure \ref{fig:i2c-timing-param} are the timing diagram and corresponding parameters of the I2C protocol. SCL\_FSM generates the SCL clock timing sequence conforming to the I2C protocol.
...
Note that operations may differ between the I2C master controller in \chipname{} and other masters or slaves on the bus. Please refer to datasheets of individual I2C devices for specific information.

\subsection{Clock Configuration}
Registers, TX RAM, and RX RAM are configured and accessed in the APB\_CLK clock domain. The main logic of the I2C master controller, including SCL\_FSM, SCL\_MAIN\_FSM, SCL\_FILTER, SDA\_FILTER, and DATA\_SHIFTER, are in the I2C\_SCLK clock domain.


You can choose the clock source for I2C\_SCLK from XTAL\_CLK or RC\_FAST\_CLK via \hyperref[fielddesc:I2CSCLKSEL]{I2C\_SCLK\_SEL}. When \hyperref[fielddesc:I2CSCLKSEL]{I2C\_SCLK\_SEL} is cleared, the clock source is XTAL\_CLK. When \hyperref[fielddesc:I2CSCLKSEL]{I2C\_SCLK\_SEL} is set, the clock source is RC\_FAST\_CLK. The clock source is enabled by configuring \hyperref[fielddesc:I2CSCLKACTIVE]{I2C\_SCLK\_ACTIVE} as high level, which passes through a fractional divider to generate I2C\_SCLK according to the following equation:
\[
...
XTAL\_CLK supports both 26 MHz and 40 MHz crystal frequencies. The frequency of RC\_FAST\_CLK is 17.5 MHz. Limited by timing parameters, the derived clock I2C\_SCLK should operate at a frequency 20 timers larger than SCL's frequency.
...
Usually when the I2C bus is idle, the SCL line is held high. The I2C master controller in \chipname{} can be programmed to generate SCL pulses in idle state. If the \hyperref[fielddesc:I2CSCLRSTSLVEN]{I2C\_SCL\_RST\_SLV\_EN} bit is set, hardware will send \hyperref[fielddesc:I2CSCLRSTSLVNUM]{I2C\_SCL\_RST\_SLV\_NUM} SCL pulses. When software reads 0 in  \hyperref[fielddesc:I2CSCLRSTSLVEN]{I2C\_SCL\_RST\_SLV\_EN} (this bit is cleared automatically by hardware), set \hyperref[fielddesc:I2CCONFUPGATE]{I2C\_CONF\_UPGATE} to stop this function.
...
\caption{I2C Registers that Need Synchronization}
...
Specifies the interval between pulling SDA low and pulling SCL low when the master generates a START condition. This interval is (\hyperref[fielddesc:I2CSCLSTARTHOLDTIME]{I2C\_SCL\_START\_HOLD\_TIME} +1) in I2C\_SCLK cycles.

\item \hyperref[fielddesc:I2CSCLLOWPERIOD]{I2C\_SCL\_LOW\_PERIOD}:
Specifies the low period of SCL. This period lasts (\hyperref[fielddesc:I2CSCLLOWPERIOD]{I2C\_SCL\_LOW\_PERIOD} + 1) in I2C\_SCLK cycles. However, it could be extended when SCL is pulled low by peripheral devices or by an END command executed by the I2C master controller, or when the clock is stretched.

\item \hyperref[fielddesc:I2CSCLWAITHIGHPERIOD]{I2C\_SCL\_WAIT\_HIGH\_PERIOD}:
Specifies time for SCL to go high in I2C\_SCLK cycles. Please make sure that SCL could be pulled high within this time period. Otherwise, the high period of SCL may be incorrect.

\item \hyperref[fielddesc:I2CSCLHIGHPERIOD]{I2C\_SCL\_HIGH\_PERIOD}:
Specifies the high period of SCL in I2C\_SCLK cycles.
...
Specifies the interval between the rising edge of SCL and the level sampling time of SDA. It is advised to set a value in the middle of SCL's high period, so as to correctly sample the level of SCL.

\item \hyperref[fielddesc:I2CSDAHOLDTIME]{I2C\_SDA\_HOLD\_TIME}:
Specifies the interval between changing the SDA output level and the falling edge of SCL.
\end{enumerate}
...
When SCL\_MAIN\_FSM remains unchanged for more than 2$^{\hyperref[fielddesc:I2CSCLMAINSTTOI2C]{I2C\_SCL\_MAIN\_ST\_TO\_I2C}}$ clock cycles, an \\I2C\_SCL\_MAIN\_ST\_TO\_INT interrupt is triggered, and then SCL\_MAIN\_FSM goes to idle state. The value of \hyperref[fielddesc:I2CSCLMAINSTTOI2C]{I2C\_SCL\_MAIN\_ST\_TO\_I2C} should be less than or equal to 22, which means SCL\_MAIN\_FSM could remain unchanged for 2$^{22}$ I2C\_SCLK clock cycles at most before the interrupt is generated.
...
Figure \ref{fig:i2c-cmd-structure} illustrates the structure of command registers. Fields of command registers are:
...
\item op\_code: Indicates the command. The I2C master controller supports five commands:

\begin{itemize}
    \item RSTART: op\_code = 6. The I2C master controller sends a START bit or a RSTART bit defined by the I2C protocol.

    \item WRITE: op\_code = 1. The I2C master controller sends a slave address, a register address  (only in double addressing mode) and data to the slave.

    \item READ: op\_code = 3. The I2C master controller reads data from the slave.

    \item STOP: op\_code = 2. The I2C master controller sends a STOP bit defined by the I2C protocol. This code also indicates that the command sequence has been executed, and the CMD\_Controller stops reading commands. After restarted by software, the CMD\_Controller resumes reading commands from command register 0.

    \item END: op\_code = 4. The I2C master controller pulls the SCL line low and suspends I2C communication. This code also indicates that the command sequence has completed, and the CMD\_Controller stops executing commands. Once software refreshes data in command registers and the RAM, the CMD\_Controller can be restarted to execute commands from command register 0 again.
\end{itemize}

\item ack\_value: Used to configure the level of the ACK bit sent by the I2C master controller during a read operation. This bit is ignored in RSTART, STOP, END and WRITE conditions.

\item ack\_exp: Used to configure the level of the ACK bit expected by the I2C master controller during a write operation. This bit is ignored during RSTART, STOP, END and READ conditions.

\item ack\_check\_en: Used to enable the I2C master controller during a write operation to check whether the ACK level sent by the slave matches ack\_exp in the command. If this bit is set and the level received does not match ack\_exp in the WRITE command, the master will generate an I2C\_NACK\_INT interrupt and a STOP condition for data transfer. If this bit is cleared, the controller will not check the ACK level sent by the slave. This bit is ignored during RSTART, STOP, END and READ conditions.
...
Both TX RAM and RX RAM are 16 × 8 bits, and can be accessed in FIFO or non-FIFO mode. If \hyperref[fielddesc:I2CNONFIFOEN]{I2C\_NONFIFO\_EN} bit is cleared, both RAMs are accessed in FIFO mode; if \hyperref[fielddesc:I2CNONFIFOEN]{I2C\_NONFIFO\_EN} bit is set, both RAMs are accessed in non-FIFO mode.
...
In FIFO mode, TX RAM of a master may wrap around to send data larger than the FIFO depth (for \chipname{} the depth is 16 bytes). Set \hyperref[fielddesc:I2CFIFOPRTEN]{I2C\_FIFO\_PRT\_EN}. If the size of data to be sent is smaller than \hyperref[fielddesc:I2CTXFIFOWMTHRHD]{I2C\_TXFIFO\_WM\_THRHD}, an I2C\_TXFIFO\_WM\_INT interrupt is generated. After receiving the interrupt, software continues writing to \hyperref[regdesc:I2CDATAREG]{I2C\_DATA\_REG}. Please ensure that software writes to or refreshes TX RAM before the master sends data, otherwise it may result in unpredictable consequences.

In FIFO mode, RX RAM of a slave may also wrap around to receive data larger than  the FIFO depth (for \chipname{} the depth is 16 bytes). Set \hyperref[fielddesc:I2CFIFOPRTEN]{I2C\_FIFO\_PRT\_EN} and clear \hyperref[fielddesc:I2CRXFULLACKLEVEL]{I2C\_RX\_FULL\_ACK\_LEVEL}. If data already received (to be overwritten) is larger than \hyperref[fielddesc:I2CRXFIFOWMTHRHD]{I2C\_RXFIFO\_WM\_THRHD}, an I2C\_RXFIFO\_WM\_INT interrupt is generated. After receiving the interrupt, software continues reading from \hyperref[regdesc:I2CDATAREG]{I2C\_DATA\_REG}.
...
Besides 7-bit addressing, the \chipname{} I2C also supports 10-bit addressing and double addressing. 10-bit addressing can be mixed with 7-bit addressing.
...
In 7-bit addressing mode, the master only needs to send one byte of address, which comprises SLV\_ADDR[6:0] and a $R/\overline W$ bit. In 7-bit addressing mode, there is a special case called general call addressing (broadcast). When the master sends the general call address (0x00) and the $R/\overline W$ bit is 0, slaves that support general call addressing respond to the master regardless of their own address.

In 10-bit addressing mode, the master needs to send two bytes of address. The first byte is slave\_addr\_first\_7bits followed by a $R/\overline W$ bit, and slave\_addr\_first\_7bits should be configured as (0x78 | SLV\_ADDR[9:8]). The second byte is slave\_addr\_second\_byte, which should be configured as SLV\_ADDR[7:0]. Since a 10-bit slave address has one more byte than a 7-bit address, byte\_num of the WRITE command and the number of bytes in the RAM increase by one.

Some I2C slaves support double addressing, where the first address is the address of an I2C slave, and the second one is the slave's memory address. \chipname{} I2C also supports double addressing.



\subsection{Starting of the I2C Master Controller}\label{subsubsec:i2c-start}
To start the I2C master controller, after configuring the controller to master mode (\hyperref[fielddesc:I2CMSMODE]{I2C\_MS\_MODE}) and command registers, write 1 to \hyperref[fielddesc:I2CTRANSSTART]{I2C\_TRANS\_START} in order that the master starts to parse and execute command sequences. The master always executes a command sequence starting from command register 0 to a STOP or an END at the end. To execute another command sequence starting from command register 0, refresh commands by writing 1 again to \hyperref[fielddesc:I2CTRANSSTART]{I2C\_TRANS\_START}.

\section{Programming Example}
This section provides programming examples for typical communication scenarios. \chipname{} has one I2C master controller. For the convenience of description, the I2C master in subsequent figures is \chipname{}'s I2C master controller, and the I2C slave are controllers compliant with \href{https://www.csd.uoc.gr/~hy428/reading/i2c_spec.pdf}{The I2C-bus specification} Version 2.1 and have corresponding functions. I2C master is referred to as I2C$_\text{master}$, and I2C slave is referred to as I2C$_\text{slave}$.
...
Figure \ref{fig:i2c-mws7} shows how I2C$_\text{master}$ writes N bytes of data to I2C$_\text{slave}$'s registers and RAM using 7-bit addressing. As shown in figure \ref{fig:i2c-mws7} , the first byte in the RAM of I2C$_\text{master}$ is a 7-bit I2C$_\text{slave}$ address followed by a $R/\overline W$ bit. When the $R/\overline W$ bit is 0, it indicates a WRITE operation. The remaining bytes are used to store data ready for transfer. The cmd box contains related command sequences.
...
\item Configure the timing parameter registers of I2C$_\text{master}$ and I2C$_\text{slave}$ according to Section \ref{subsec:i2c-timing-para}. Adjust the timing of I2C$_\text{slave}$ according to its manual.
\item Set \hyperref[fielddesc:I2CMSMODE]{I2C\_MS\_MODE}  to 1.
\item Write 1 to \hyperref[fielddesc:I2CCONFUPGATE]{I2C\_CONF\_UPGATE} to synchronize registers.
...
\hyperref[fielddesc:I2CCOMMAND0]{I2C\_COMMAND0} & RSTART& ---&---&---&---  \\ \hline
\hyperref[fielddesc:I2CCOMMAND1]{I2C\_COMMAND1} & WRITE& ack\_value&ack\_exp&1&N+1  \\ \hline
\hyperref[fielddesc:I2CCOMMAND2]{I2C\_COMMAND2} & STOP& ---&---&---&---  \\ \hline
\end{longtable}
\item Write I2C$_\text{slave}$ address and data to be sent to TX RAM of I2C$_\text{master}$ in either FIFO mode or non-FIFO mode according to Section \ref{subsubsec:i2c-txrx}.
\item Set the address of I2C$_\text{slave}$ via I2C\_SLAVE\_ADDR[7:0].
\item Write 1 to \hyperref[fielddesc:I2CTRANSSTART]{I2C\_TRANS\_START} to start transfer, and enable I2C$_\text{slave}$.
\item I2C$_\text{slave}$ compares the slave address sent by I2C$_\text{master}$ with its own address. When ack\_check\_en in I2C$_\text{master}$'s WRITE command is 1, I2C$_\text{master}$ checks ACK value each time it sends a byte. When ack\_check\_en  is 0, I2C$_\text{master}$ does not check ACK value and take I2C$_\text{slave}$ as a matching slave by default.
\begin{itemize}
\item Match: If the received ACK value matches ack\_exp (the expected ACK value), I2C$_\text{master}$ continues data transfer.
\item Not match: If the received ACK value does not match ack\_exp, I2C$_\text{master}$ generates an I2C\_NACK\_INT  interrupt and stops data transfer.
\end{itemize}

\item I2C$_\text{master}$ sends data, and checks ACK value or not according to ack\_check\_en.
\item If data to be sent (N) is larger than the depth of TX FIFO, TX RAM of I2C$_\text{master}$ may wrap around in FIFO mode. For details, please refer to Section \ref{subsubsec:i2c-txrx}.

\item After data transfer completes, I2C$_\text{master}$ executes the STOP command, and generates an I2C\_TRANS\_COMPLETE\_INT  interrupt.
...
\hyperref[fielddesc:I2CCOMMAND0]{I2C\_COMMAND0} & RSTART& ---&---&---&---  \\ \hline
\hyperref[fielddesc:I2CCOMMAND1]{I2C\_COMMAND1} & WRITE& ack\_value&ack\_exp&1&N+2  \\ \hline
\hyperref[fielddesc:I2CCOMMAND2]{I2C\_COMMAND2} & STOP& ---&---&---&---  \\ \hline
\end{longtable}

\item Set the address of I2C$_\text{slave}$ via I2C\_SLAVE\_ADDR[9:0].
\item Write I2C$_\text{slave}$ address and data to be sent to TX RAM of I2C$_\text{master}$. The first byte of I2C$_\text{slave}$ address comprises ((0x78 | I2C\_SLAVE\_ADDR[9:8])<<1) and a $R/\overline W$ bit. The second byte of I2C$_\text{slave}$ address is I2C\_SLAVE\_ADDR[7:0]. These two bytes are followed by data to be sent in FIFO or non-FIFO mode.
\item Write 1 to \hyperref[fielddesc:I2CCONFUPGATE]{I2C\_CONF\_UPGATE} to synchronize registers.
\item Write 1 to \hyperref[fielddesc:I2CTRANSSTART]{I2C\_TRANS\_START} to start transfer, and enable I2C$_\text{slave}$.
\item I2C$_\text{slave}$ compares the slave address sent by I2C$_\text{master}$ with its own address. When ack\_check\_en in I2C$_\text{master}$'s WRITE command is 1, I2C$_\text{master}$ checks ACK value each time it sends a byte. When ack\_check\_en is 0, I2C$_\text{master}$ does not check ACK value and take I2C$_\text{slave}$ as matching slave by default.
\begin{itemize}
\item Match: If the received ACK value matches ack\_exp (the expected ACK value), I2C$_\text{master}$ continues data transfer.
\item Not match: If the received ACK value does not match ack\_exp, I2C$_\text{master}$ generates an I2C\_NACK\_INT interrupt and stops data transfer.
\end{itemize}
...
\item I2C$_\text{master}$ sends data, and checks ACK value or not according to ack\_check\_en.
\item If data to be sent is larger than the depth of TX FIFO, TX RAM of I2C$_\text{master}$ may wrap around in FIFO mode. For details, please refer to Section \ref{subsubsec:i2c-txrx}.
...
Figure \ref{fig:i2c-mws7-double} shows how I2C$_\text{master}$ writes N bytes of data to I2C$_\text{slave}$'s registers or RAM using 7-bit double addressing. The configuration and transfer process is similar to what is described in Section \ref{subsec:i2c-mws7}, except that in 7-bit double addressing mode I2C$_\text{master}$ sends two 7-bit addresses. The first address is the address of an I2C slave, and the second one is I2C$_\text{slave}$'s memory address (i.e. addrM in Figure \ref{fig:i2c-mws7-double} on the right). When using double addressing, RAM must be accessed in non-FIFO mode.
The I2C slave put received byte0 \verb+~+ byte(N-1) into its registers or RAM in an order staring from addrM.

\subsubsection{Configuration Example}
\begin{enumerate}
\item Choose an I2C$_\text{slave}$ that supports double addressing mode and enable this mode.
\item Set \hyperref[fielddesc:I2CMSMODE]{I2C\_MS\_MODE} to 1.
\item Write 1 to \hyperref[fielddesc:I2CCONFUPGATE]{I2C\_CONF\_UPGATE} to synchronize registers.
\item Configure command registers of I2C$_\text{master}$.
\begin{longtable}{ | p{4cm} | p{2cm} | p{2cm} | p{2cm} |p{2cm} | p{2cm} |}
...
\hyperref[fielddesc:I2CCOMMAND2]{I2C\_COMMAND2} & STOP& ---&---&---&---  \\ \hline
\end{longtable}
\item Write I2C$_\text{slave}$ address and data to be sent to TX RAM of I2C$_\text{master}$ in FIFO or non-FIFO mode.
\item Set the address of I2C$_\text{slave}$ via I2C\_SLAVE\_ADDR[7:0].
\item Write 1 to \hyperref[fielddesc:I2CCONFUPGATE]{I2C\_CONF\_UPGATE} to synchronize registers.
\item Write 1 to \hyperref[fielddesc:I2CTRANSSTART]{I2C\_TRANS\_START} to start transfer, and enable I2C$_\text{slave}$.
\item I2C$_\text{slave}$ compares the slave address sent by I2C$_\text{master}$ with its own address. When ack\_check\_en in I2C$_\text{master}$'s WRITE command is 1, I2C$_\text{master}$ checks ACK value each time it sends a byte. When ack\_check\_en is 0, I2C$_\text{master}$ does not check ACK value and take I2C$_\text{slave}$ as matching slave by default.
...
Note that other I2C$_\text{master}$ devices will not transact on the bus between two segments. The bus is only released after a STOP signal is sent. The I2C master controller can be reset by setting \hyperref[fielddesc:I2CFSMRST]{I2C\_FSM\_RST} field at any time. This field will later be cleared automatically by hardware.
...
\hyperref[fielddesc:I2CCOMMAND0]{I2C\_COMMAND0} & RSTART& ---&---&---&---  \\ \hline
\hyperref[fielddesc:I2CCOMMAND1]{I2C\_COMMAND1} & WRITE& ack\_value&ack\_exp&1&N+1  \\ \hline
\hyperref[fielddesc:I2CCOMMAND2]{I2C\_COMMAND2} & END& ---&---&---&---  \\ \hline
\end{longtable}
\item Write I2C$_\text{slave}$ address and data to be sent to TX RAM of I2C$_\text{master}$ in either FIFO mode or non-FIFO mode according to Section \ref{subsubsec:i2c-txrx}.
\item Set the address of I2C$_\text{slave}$ via I2C\_SLAVE\_ADDR[7:0].
\item Write 1 to \hyperref[fielddesc:I2CCONFUPGATE]{I2C\_CONF\_UPGATE} to synchronize registers.
...
\item After the I2C\_END\_DETECT\_INT interrupt is generated, set \hyperref[fielddesc:I2CENDDETECTINTCLR]{I2C\_END\_DETECT\_INT\_CLR} to 1 to clear this interrupt.
\item Update I2C$_\text{master}$'s command registers.
\begin{longtable}{ | p{4cm} | p{2cm} | p{2cm} | p{2cm} |p{2cm} | p{2cm} |}
\hline\rowcolor{lightgray}
Command registers& op\_code & ack\_value&ack\_exp&ack\_check\_en&byte\_num  \\ \hline
\hyperref[fielddesc:I2CCOMMAND0]{I2C\_COMMAND0} & WRITE& ack\_value&ack\_exp&1&M  \\ \hline
\hyperref[fielddesc:I2CCOMMAND1]{I2C\_COMMAND1} & END/STOP& ---&---&---&---  \\ \hline
\end{longtable}
\item Write M bytes of data to be sent to TX RAM of I2C$_\text{master}$ in FIFO or non-FIFO mode.
\item Write 1 to \hyperref[fielddesc:I2CTRANSSTART]{I2C\_TRANS\_START} bit to start transfer and repeat step 9.
\item If the command is a STOP, I2C stops transfer and generates an I2C\_TRANS\_COMPLETE\_INT interrupt.
\item If the command is an END, repeat step 10.
\item Update I2C$_\text{master}$'s command registers.
\begin{longtable}{ | p{4cm} | p{2cm} | p{2cm} | p{2cm} |p{2cm} | p{2cm} |}
\hline\rowcolor{lightgray}
Command registers & op\_code & ack\_value&ack\_exp&ack\_check\_en&byte\_num  \\ \hline
\hyperref[fielddesc:I2CCOMMAND1]{I2C\_COMMAND1} & STOP& ---&---&---&---  \\ \hline
\end{longtable}
\item Write 1 to \hyperref[fielddesc:I2CTRANSSTART]{I2C\_TRANS\_START}  bit to start transfer.
\item I2C$_\text{master}$ executes the STOP command and generates an I2C\_TRANS\_COMPLETE\_INT  interrupt.
...
Figure \ref{fig:i2c-mrs7} shows how I2C$_\text{master}$ reads N bytes of data from an I2C slave's registers or RAM using 7-bit addressing. cmd1 is a WRITE command, and when this command is executed, I2C$_\text{master}$ sends I2C$_\text{slave}$ address. The byte sent comprises a 7-bit I2C$_\text{slave}$ address and a $R/\overline W$ bit. When the $R/\overline W$ bit is 1, it indicates a READ operation. If the address of an I2C slave matches the sent address, this matching slave starts sending data to I2C$_\text{master}$. I2C$_\text{master}$ generates acknowledgements according to ack\_value defined in the READ command upon receiving a byte.

As illustrated in Figure \ref{fig:i2c-mrs7}, I2C$_\text{master}$ executes two READ commands: it generates ACKs for (N-1) bytes of data in cmd2, and a NACK for the last byte of data in cmd3. This configuration may be changed as required. I2C$_\text{master}$ writes received data into the controller RAM from addr0, whose original content (a I2C$_\text{slave}$ address and a $R/\overline W$ bit) is overwritten by byte0 marked red in Figure \ref{fig:i2c-mrs7}.
...
Command registers & op\_code & ack\_value&ack\_exp&ack\_check\_en&byte\_num  \\ \hline
\hyperref[fielddesc:I2CCOMMAND0]{I2C\_COMMAND0} & RSTART& ---&---&---&---  \\ \hline
\hyperref[fielddesc:I2CCOMMAND1]{I2C\_COMMAND1} & WRITE& 0&0&1&1  \\ \hline
\hyperref[fielddesc:I2CCOMMAND2]{I2C\_COMMAND2} & READ& 0&0&1&N-1  \\ \hline
\hyperref[fielddesc:I2CCOMMAND3]{I2C\_COMMAND3} & READ& 1&0&1&1  \\ \hline
\hyperref[fielddesc:I2CCOMMAND4]{I2C\_COMMAND4} & STOP& ---&---&---&---  \\ \hline
\end{longtable}
\item Write I2C$_\text{slave}$ address to TX RAM of I2C$_\text{master}$ in either FIFO mode or non-FIFO mode according to Section \ref{subsubsec:i2c-txrx}.
\item Set the address of I2C$_\text{slave}$ via I2C\_SLAVE\_ADDR[7:0].
\item Write 1 to \hyperref[fielddesc:I2CCONFUPGATE]{I2C\_CONF\_UPGATE} to synchronize registers.
\item Write 1 to \hyperref[fielddesc:I2CTRANSSTART]{I2C\_TRANS\_START} bit to start transfer, and enable I2C$_\text{slave}$.
...
\item I2C$_\text{slave}$ sends data, and I2C$_\text{master}$ sends ACK value according to ack\_check\_en in the READ command.

\item If data to be received (N) is larger than the depth of RX FIFO, RX RAM of I2C$_\text{master}$ may wrap around in FIFO mode. For details, please refer to Section \ref{subsubsec:i2c-txrx}.

\item After I2C$_\text{master}$ has received the last byte of data, set ack\_value to 1. I2C$_\text{slave}$ will stop transfer once receiving the I2C\_NACK\_INT interrupt.
...
Command registers & op\_code & ack\_value&ack\_exp&ack\_check\_en&byte\_num  \\ \hline
\hyperref[fielddesc:I2CCOMMAND0]{I2C\_COMMAND0} & RSTART& ---&---&---&---  \\ \hline
\hyperref[fielddesc:I2CCOMMAND1]{I2C\_COMMAND1} & WRITE& 0&0&1&2  \\ \hline
\hyperref[fielddesc:I2CCOMMAND2]{I2C\_COMMAND2} & RSTART& ---&---&---&---  \\ \hline
\hyperref[fielddesc:I2CCOMMAND3]{I2C\_COMMAND3} & WRITE& 0&0&1&1  \\ \hline
\hyperref[fielddesc:I2CCOMMAND4]{I2C\_COMMAND4} & READ& 0&0&1&N-1  \\ \hline
\hyperref[fielddesc:I2CCOMMAND5]{I2C\_COMMAND5} & READ& 1&0&1&1  \\ \hline
\hyperref[fielddesc:I2CCOMMAND6]{I2C\_COMMAND6} & STOP& ---&---&---&---  \\ \hline
\end{longtable}

\item Set the 10-bit address of I2C$_\text{slave}$ via I2C\_SLAVE\_ADDR[9:0].
\item Write I2C$_\text{slave}$ address and data to be sent to TX RAM of I2C$_\text{master}$ in either FIFO or non-FIFO mode. The first byte of address comprises ((0x78 | I2C\_SLAVE\_ADDR[9:8])<<1) and a $R/\overline W$ bit, which is 1 and indicates a WRITE operation. The second byte of address is I2C\_SLAVE\_ADDR[7:0]. The third byte is ((0x78 | I2C\_SLAVE\_ADDR[9:8])<<1) and a $R/\overline W$ bit, which is 1 and indicates a READ operation.
...
\item I2C$_\text{master}$ sends a RSTART and the third byte in TX RAM, which is ((0x78 | I2C\_SLAVE\_ADDR[9:8])<<1) and a $R/\overline W$ bit that indicates READ.
\item I2C$_\text{slave}$ repeats step 8. If its address matches the address sent by I2C$_\text{master}$, I2C$_\text{slave}$ proceed on to the next steps.
...
Figure \ref{fig:i2c-mrs7-m} shows how I2C$_\text{master}$ reads data from specified addresses in I2C slave's registers or RAM. I2C$_\text{master}$ sends two bytes of addresses: the first byte is a 7-bit I2C$_\text{slave}$ address followed by a $R/\overline W$ bit, which is 0 and indicates a WRITE; the second byte is I2C$_\text{slave}$'s memory address addrM. After a RSTART condition, I2C$_\text{master}$ sends the first byte of address again, but the $R/\overline W$ bit is 1 which indicates a READ. Then, I2C$_\text{master}$ reads data starting from addrM.
...
\item Set the address of I2C$_\text{slave}$ via I2C\_SLAVE\_ADDR[7:0].
\item Write I2C$_\text{slave}$ address and data to be sent to TX RAM of I2C$_\text{master}$ in either FIFO or non-FIFO mode according to Section \ref{subsubsec:i2c-txrx}. The first byte of address comprises ( I2C\_SLAVE\_ADDR[6:0])<<1) and a $R/\overline W$ bit, which is 0 and indicates a WRITE. The second byte of address is memory address M of I2C$_\text{slave}$. The third byte is ( I2C\_SLAVE\_ADDR[6:0])<<1) and a $R/\overline W$ bit, which is 1 and indicates a READ.
...
\item I2C$_\text{master}$ sends a RSTART and the third byte in TX RAM, which is ((0x78 | I2C\_SLAVE\_ADDR[6:0])<<1)and an R bit.
\item I2C$_\text{slave}$ repeats step 9. If its address matches the address sent by I2C$_\text{master}$, I2C$_\text{slave}$ proceed on to the next steps.
...
\hyperref[fielddesc:I2CCOMMAND0]{I2C\_COMMAND0} & RSTART& ---&---&---&---  \\ \hline
\hyperref[fielddesc:I2CCOMMAND1]{I2C\_COMMAND1} & WRITE& 0&0&1&1  \\ \hline
\hyperref[fielddesc:I2CCOMMAND2]{I2C\_COMMAND2} & READ& 0&0&1&N  \\ \hline
\hyperref[fielddesc:I2CCOMMAND3]{I2C\_COMMAND3} & END& ---&---&---&---  \\ \hline
\end{longtable}
\item Write I2C$_\text{slave}$ address to TX RAM of I2C$_\text{master}$ in FIFO or non-FIFO mode.
\item Set the address of I2C$_\text{slave}$ via I2C\_SLAVE\_ADDR[7:0].
\item Write 1 to \hyperref[fielddesc:I2CCONFUPGATE]{I2C\_CONF\_UPGATE} to synchronize registers.
\item Write 1 to \hyperref[fielddesc:I2CTRANSSTART]{I2C\_TRANS\_START}  to start transfer, and enable I2C$_\text{slave}$.
...
\item Once finishing reading data in the first READ command, I2C$_\text{master}$ executes the END command and triggers an I2C\_END\_DETECT\_INT interrupt, which is cleared by setting \hyperref[fielddesc:I2CENDDETECTINTCLR]{I2C\_END\_DETECT\_INT\_CLR} to 1.
\item Update I2C$_\text{master}$'s command registers using one of the following two methods:
\begin{longtable}{ | p{4cm} | p{2cm} | p{2cm} | p{2cm} |p{2cm} | p{2cm} |}
\hline\rowcolor{lightgray}
Command registers & op\_code & ack\_value&ack\_exp&ack\_check\_en&byte\_num  \\ \hline
\hyperref[fielddesc:I2CCOMMAND0]{I2C\_COMMAND0} & READ& ack\_value&ack\_exp&1&M  \\ \hline
\hyperref[fielddesc:I2CCOMMAND1]{I2C\_COMMAND1} & END& ---&---&---&---  \\ \hline
\end{longtable}
Or
\begin{longtable}{ | p{4cm} | p{2cm} | p{2cm} | p{2cm} |p{2cm} | p{2cm} |}
\hline\rowcolor{lightgray}
Command registers & op\_code & ack\_value&ack\_exp&ack\_check\_en&byte\_num  \\ \hline
\hyperref[fielddesc:I2CCOMMAND0]{I2C\_COMMAND0} & READ& 0&0&1&M-1  \\ \hline
\hyperref[fielddesc:I2CCOMMAND1]{I2C\_COMMAND1} & READ& 1&0&1&1  \\ \hline
\hyperref[fielddesc:I2CCOMMAND2]{I2C\_COMMAND2} & STOP& ---&---&---&---  \\ \hline
\end{longtable}


\item Prepare data that I2C$_\text{slave}$ will send.
\item Write 1 to \hyperref[fielddesc:I2CTRANSSTART]{I2C\_TRANS\_START}  bit to start transfer and repeat step 9.
\item If the last command is a STOP, then set ack\_value to 1 after I2C$_\text{master}$ has received the last byte of data. I2C$_\text{slave}$ stops transfer upon the I2C\_NACK\_INT interrupt. I2C$_\text{master}$ executes the STOP command to stop transfer and generates an I2C\_TRANS\_COMPLETE\_INT interrupt.
\item If the last command is an END, then repeat step 11 and proceed on to the next steps.
\item Update I2C$_\text{master}$'s command registers.
\begin{longtable}{ | p{4cm} | p{2cm} | p{2cm} | p{2cm} |p{2cm} | p{2cm} |}
\hline\rowcolor{lightgray}
Command registers & op\_code & ack\_value&ack\_exp&ack\_check\_en&byte\_num  \\ \hline
...
\end{longtable}
\item Write 1 to \hyperref[fielddesc:I2CTRANSSTART]{I2C\_TRANS\_START}  bit to start transfer.
\item I2C$_\text{master}$ executes the STOP command to stop transfer, and generates an I2C\_TRANS\_COMPLETE\_INT interrupt.
...
\item \label{int:i2c-rxfifo-udf}I2C\_RXFIFO\_UDF\_INT: Triggered when the I2C master controller reads RX FIFO via the APB bus, but RX FIFO is empty.
\item \label{int:i2c-txfifo-ovf}I2C\_TXFIFO\_OVF\_INT: Triggered when the I2C master controller writes TX FIFO via the APB bus, but TX FIFO is full.
...
\item \label{int:i2c-trans-start}I2C\_TRANS\_START\_INT: Triggered when the I2C master controller sends a START bit.
...
\item \label{int:i2c-trans-complete}I2C\_TRANS\_COMPLETE\_INT: Triggered when the I2C master controller detects a STOP bit.
...
\item \label{int:i2c-byte-trans-done}I2C\_BYTE\_TRANS\_DONE\_INT: Triggered when the I2C master controller sends or receives a byte.
...
\item \label{int:i2c-rxfifo-ovf}I2C\_RXFIFO\_OVF\_INT: Triggered when RX FIFO of the I2C master controller overflows.
...
The addresses in this section are relative to \textcolor{red}{I2C Master controller} base address provided in Table \ref{tab:sysmem-base-address} in Chapter \ref{mod:sysmem} \textit{\nameref{mod:sysmem}}.

The abbreviations given in Column \textbf{Access} are explained in Section \hyperref[glossary-access-types]{\textit{Access Types for Registers}}.

\subfile{\modulefiles/23-i2c-reg-summ__EN}
...
\subfile{\modulefiles/23-i2c-reg__EN}
