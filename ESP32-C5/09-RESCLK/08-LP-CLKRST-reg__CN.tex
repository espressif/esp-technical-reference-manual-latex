...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:LPPERIRNGCKEN}\item [LPPERI\_RNG\_CK\_EN] 配置是否使能 RNG 模块的时钟门控。 \\
0：禁止时钟信号通过\\
1：使能时钟信号通过\\(R/W)
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:LPPERIRNGRESETEN}\item [LPPERI\_RNG\_RESET\_EN] 配置 RNG 控制器模块的软件复位。\\
0：不复位\\
1：复位\\(R/W)
