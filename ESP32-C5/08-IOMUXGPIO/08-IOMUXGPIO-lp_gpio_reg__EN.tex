...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:LPGPIOOUTW1TS}\item [LP\_GPIO\_OUT\_W1TS] Configures whether or not to enable the output register LP\_IO\_OUT\_REG of GPIO0$\sim$GPIO6.\\
The value of each bit can be:\\
0: Not set\\
...
Bit[0]$\sim$bit[6] are corresponding to GPIO0$\sim$GPIO6.\\
This field is used together with LP\_GPIO\_PIN\regindex{n}\_INT\_TYPE in register LP\_GPIO\_PIN\regindex{n}\_REG.\\
 (R/W/WTC)
\end{reglist}\end{regdesc}
\end{register}
...
0: Not enable\\
1: Enable\\
This function is disabled when PD\_LP\_PERI is powered off.\\ (R/W)
\end{reglist}\end{regdesc}
\end{register}
