...
\caption{Typical AES 模式下状态返回值}
\label{tab:aes-state-value-typical}
\\\hline
\rowcolor{lightgray}
\textbf{\hyperref[regdesc:AESSTATEREG]{AES\_STATE\_REG}} & \textbf{描述} &  \textbf{状态说明} \\ \hline
...
在 Typical AES 工作模式下，AES 加速器可以使用密钥对 128 位的块进行加解密。在操作寄存器 \hyperref[regdesc:AESTEXTINnREG]{AES\_TEXT\_IN\_\regindex{m}\_REG} 和 \hyperref[regdesc:AESTEXTINnREG]{AES\_TEXT\_OUT\_\regindex{m}\_REG} 中的数据时，用户应遵循表 \ref{tab:aes-endianness-type-typical-plaintext-ciphertext} 中定义的文本字节序。
...
\phantom{aaaaaaaa}此处，$X_{1}、X_{2}、\cdots、X_{n-1}$ 的长度均为 128 比特，而 $X_{n}$ 的长度为 {\it{t}} (0<=$t$<=127)。\\
\phantom{aaaaaaaa}如果 ${\it{t}} = 0$，则\\
\phantom{aaaaaaaa}\phantom{空空空空空} {\bfseries TEXT-PADDING}($X$) = $X$;\\
\phantom{aaaaaaaa}如果 $0<{\it{t}} <= 127$，定义 $X_{n}^{*}$ 块的长度为 128 比特，且 $X_{n}^{*} = X_{n} || 0^{128-{\it{t}}}$，则\\
...
\section{抗攻击伪轮函数}\label{aes-pseudo-round}
...
\subfile{\modulefiles/22-AES-mem-summ__CN}
...
\subfile{\modulefiles/22-AES-reg-summ__CN.tex}
...
\subfile{\modulefiles/22-AES-reg__CN.tex}
