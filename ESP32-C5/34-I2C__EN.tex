...
You can configure the clock source for I2C\_SCLK of I2C in the main system (HP) to XTAL\_CLK or RC\_FAST\_CLK via \hyperref[fielddesc:PCRI2CSCLKSEL]{PCR\_I2C\_SCLK\_SEL}. For the clock source for I2C\_SCLK of LP\_I2C in the low-power system (LP), you can configure it to CLK\_XTALD2 or CLK\_ROOT\_FAST via \hyperref[fielddesc:LPPERILPI2CCLKSEL]{LP\_CLKRST\_LP\_I2C\_CLK\_SEL}.
...
\begin{itemize}
    \item Enable the clock source for I2C\_SCLK of LP\_I2C by configuring \hyperref[fielddesc:LPPERICKENLPI2C]{LPPERI\_LP\_EXT\_I2C\_CK\_EN} to 1.
    \item When \hyperref[fielddesc:LPCLKRSTLPI2CCLKSEL]{LP\_CLKRST\_LP\_I2C\_CLK\_SEL} is 0, the clock source is CLK\_ROOT\_FAST.
    \item When \hyperref[fielddesc:LPCLKRSTLPI2CCLKSEL]{LP\_CLKRST\_LP\_I2C\_CLK\_SEL} is 1, the clock source is CLK\_XTALD2.
\end{itemize}
...
To start the I2C controller slave mode, after configuring the controller to slave mode and setting the relevant registers, write 1 to \hyperref[fielddesc:I2CTRANSSTART]{I2C\_TRANS\_START}. The slave will respond to the master when the received address matches.

For the slave device, please note:

\begin{itemize}
    \item Always set \hyperref[fielddesc:I2CTRANSSTART]{I2C\_TRANS\_START} before accepting any transfer. Otherwise, even if the address matches, the slave will not respond to the master.
    \item If the master initiates a STOP condition and prematurely terminates the transmission with the slave, the slave must first set \hyperref[fielddesc:I2CSLVTXAUTOSTARTEN]{I2C\_SLV\_TX\_AUTO\_START\_EN}, then set \hyperref[fielddesc:I2CTRANSSTART]{I2C\_TRANS\_START}, and finally clear \hyperref[fielddesc:I2CSLVTXAUTOSTARTEN]{I2C\_SLV\_TX\_AUTO\_START\_EN} before the next transfer. Otherwise, an abnormal TX FIFO pointer issue will occur in the slave.
\end{itemize}
...
\begin{itemize}
\item The size of TX/RX RAM in LP\_I2C is 16x8 bit, which means the TX／RX FIFO depth is 16 bytes.
\item The clock source of APB\_CLK in LP\_I2C is CLK\_AON\_FAST. The clock source for I2C\_SCLK is configured via \hyperref[fielddesc:LPI2CSCLKSEL]{LP\_CLKRST\_LP\_I2C\_CLK\_SEL}. The corresponding bits are:
    \begin{itemize}
        \item 0: CLK\_ROOT\_FAST
        \item 1: CLK\_XTALD2
    \end{itemize}
    Configuring \hyperref[fielddesc:LPPERILPEXTI2CCKEN]{LPPERI\_LP\_EXT\_I2C\_CK\_EN} to 1 enables the clock source of I2C\_SCLK. Adjust the timing registers accordingly when the clock frequency changes.
\end{itemize}

See the programming examples of \chipname{} I2C$_\text{slave}$ in Section \ref{subsec:i2c-prom-guide} for that of LP\_I2C.
...
\item \label{int:i2c-general-call}I2C\_GENERAL\_CALL\_INT: Triggered when \href{https://www.csd.uoc.gr/~hy428/reading/i2c_spec.pdf#page=16}{the general call address} is received in slave mode.
\item \label{int:i2c-slave-addr-unmatch}I2C\_SLAVE\_ADDR\_UNMATCH\_INT: Triggered when the received slave address is inconsistent with the internally configured slave address in slave mode.
\end{itemize}
...
\subfile{\modulefiles/34-I2C-reg-summ__EN.tex}
...
\subfile{\modulefiles/34-I2C-LP-reg-summ__EN.tex}
...
\subfile{\modulefiles/34-I2C-reg__EN.tex}

\subsection{LP\_I2C Registers}

\subfile{\modulefiles/34-I2C-LP-reg__EN.tex}
