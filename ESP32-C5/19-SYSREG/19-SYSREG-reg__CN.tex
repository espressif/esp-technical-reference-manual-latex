...
1：开启\\
(R/W)
\label{fielddesc:HPSYSTEMENABLEDOWNLOADDBENCRYPT}\item [HP\_SYSTEM\_ENABLE\_DOWNLOAD\_DB\_ENCRYPT] 配置是否在下载 Boot 模式下开启 MSPI XTS 自动加密。\\
0：关闭\\
1：开启\\
(R/W)
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:HPSYSTEMSECDPALEVEL}\item [HP\_SYSTEM\_SEC\_DPA\_LEVEL] 配置是否开启防 DPA 攻击。仅当 \hyperref[fielddesc:HPSYSTEMSECDPACFGSEL]{HP\_SYSTEM\_SEC\_DPA\_CFG\_SEL} 置 0 时可用。\\
0：关闭\\
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:HPSYSTEMCOREDEBUGRUNSTALLENABLE}\item [HP\_SYSTEM\_CORE\_DEBUG\_RUNSTALL\_ENABLE] 配置是否在启用 HP CPU 和 LP CPU 的 RunStall 功能。一旦启用，当任一个 CPU 进入调试模式时，另一个 CPU 自动暂停工作。\\
0：关闭\\
1：开启\\
(R/W)
\label{fielddesc:HPSYSTEMCORERUNSTALLED}\item [HP\_SYSTEM\_CORE\_RUNSTALLED] 表示 HP CPU 的 RunStall 状态。\\
0：继续工作\\
1：停止工作\\
(RO)
\end{reglist}\end{regdesc}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:HPSYSTEMBITSCRAMBLERRXSEL}\item [HP\_SYSTEM\_BITSCRAMBLER\_RX\_SEL] 配置选择接入比特调节器接收通道的 DMA 外设。\\
1：GPSPI2 \\
2：UHCI0 \\
3：I2S0 \\
4：AES \\
...
其他：NONE\\
(R/W)
\label{fielddesc:HPSYSTEMBITSCRAMBLERTXSEL}\item [HP\_SYSTEM\_BITSCRAMBLER\_TX\_SEL] 配置选择接入比特调节器发送通道的 DMA 外设。\\
1：GPSPI2 \\
2：UHCI0 \\
3：I2S0 \\
4：AES \\
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:HPSYSTEMCPUPERITIMEOUTUID}\item [HP\_SYSTEM\_CPU\_PERI\_TIMEOUT\_UID] 表示超时发生时的 master ID[4:0] 和 master 权限[6:5]。该寄存器将在清除超时中断后清除。 (WTC)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:HPSYSTEMHPPERITIMEOUTUID}\item [HP\_SYSTEM\_HP\_PERI\_TIMEOUT\_UID] 表示超时发生时的 master  ID[4:0] 和 master 权限[6:5]。该寄存器将在清除超时中断后清除。 (WTC)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:LPPERIBUSTIMEOUTUID}\item [LP\_PERI\_BUS\_TIMEOUT\_UID] 表示超时发生时的 master ID[4:0] 和 master 权限[6:5]。该寄存器将在清除超时中断后清除。 (WTC)
\end{reglist}\end{regdesc}
\end{register}
