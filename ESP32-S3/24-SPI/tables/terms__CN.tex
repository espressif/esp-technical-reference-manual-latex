...
\textbf{主机模式}     & GP-SPI 用作 SPI 主机，发起 SPI 传输事务。\\ 
\textbf{从机模式}      & GP-SPI 用作 SPI 从机，当其 CS 被拉低时，与 SPI 主机进行数据传输。\\
...
\textbf{FSPI} & Fast SPI，GP-SPI2 输入输出信号的前缀。FSPI 总线信号可通过 GPIO 交换矩阵或 IO MUX 与 GPIO 管脚相连。\\
\textbf{SPI3} & GP-SPI3 输入输出信号的前缀。SPI3 总线信号仅可通过 GPIO 交换矩阵连接到 GPIO 管脚。
\end{longtable}
