...
\addtocounter{Regfloat}{-1}
\begin{register}{H}{UART\_INT\_ENA\_REG}{0x{}000C}
\begin{regdesc}\begin{reglist}
\item [Continued from the previous page...]
...
\addtocounter{Regfloat}{-1}
\begin{register}{H}{UART\_INT\_CLR\_REG}{0x{}0010}
\begin{regdesc}\begin{reglist}
\item [Continued from the previous page...]
...
\label{fielddesc:UARTIRDAWCTL}\item [UART\_IRDA\_WCTL] 0: Set IrDA transmitter's 11th bit to 0; 1: The IrDA transmitter's 11th bit is the same as 10th bit. (R/W)
...
\label{fielddesc:UARTTXFLOWEN}\item [UART\_TX\_FLOW\_EN] Set this bit to enable flow control function for transmitter. (R/W)
...
\label{fielddesc:UARTCLKEN}\item [UART\_CLK\_EN] 0: Support clock only when application writes registers; 1: Force clock on for registers. (R/W)
\label{fielddesc:UARTERRWRMASK}\item [UART\_ERR\_WR\_MASK] 0: Receiver stores the data even if the received data is wrong; 1: Receiver stops storing data into FIFO when data is wrong. (R/W)
...
0: Not add\\
1: Add\\(R/W)
\label{fielddesc:UARTRS485TXRXEN}\item [UART\_RS485TX\_RX\_EN] Set this bit to enable receiver could receive data when the transmitter is transmitting data in RS485 mode. (R/W)
...
\label{fielddesc:UARTTXRSTCORE}\item [UART\_TX\_RST\_CORE] Write 1 and then write 0 to this bit, to reset UART TX. (R/W)
\label{fielddesc:UARTRXRSTCORE}\item [UART\_RX\_RST\_CORE] Write 1 and then write 0 to this bit, to reset UART RX. (R/W)
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:UARTAPBRXRADDR}\item [UART\_APB\_RX\_RADDR] This field stores the offset address in RX FIFO when software reads data from RX FIFO via APB. UART0 is 0x{}200. UART1 is 0x{}280. UART2 is 0x{}300. (RO)
\label{fielddesc:UARTRXWADDR}\item [UART\_RX\_WADDR] This field stores the offset address in RX FIFO when Rx\_FIFO\_Ctrl writes RX FIFO. UART0 is 0x{}200. UART1 is 0x{}280. UART2 is 0x{}300. (RO)
\end{reglist}\end{regdesc}
\end{register}
...
\label{fielddesc:UARTUPDATECTRL}\item [UART\_UPDATE\_CTRL] This bit is used to control register synchronization mode. 0: After registers are configured, software needs to write 1 to UART\_REG\_UPDATE to synchronize registers; 1: Registers are automatically synchronized into UART Core's clock domain. (R/W)
