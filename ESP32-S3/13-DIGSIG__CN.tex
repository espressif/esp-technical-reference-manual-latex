...
私钥参数以密文形式存储在 flash 或其他存储器中。对其解密而使用的 \begin{math} RSA\_DS\_KEY \end{math} 只能由 RSA\_DS 外设通过 HMAC 模块获取，并且 HMAC 模块求解该密钥所需的一切输入信息（\begin{math} HMAC\_KEY \end{math}）只存放在 eFuse 中且只允许被 HMAC 模块访问。这意味着只有 RSA\_DS 硬件才能解密私钥密文，软件绝对不会获取私钥明文。关于 eFuse 和 HMAC 的相关细节请参照章节 \ref{mod:efuse} \textit{\nameref{mod:efuse}} 和 章节 \ref{mod:hmac} \textit{\nameref{mod:hmac}}。

需要签名时，软件直接将输入消息 \begin{math}X\end{math} 发送到 RSA\_DS 外设。RSA 签名操作完成后，软件将读取签名结果 \begin{math}Z\end{math}。
...
\begin{math}M^{\prime}\end{math} 与验证摘要一起由用户加密并以密文 \begin{math}C\end{math} 的形式存储。密文 \begin{math}C\end{math} 输入到 RSA\_DS 外设之后先由硬件解密，然后参与 RSA 签名运算。如何生成 \begin{math}C\end{math} 请参考第 \ref{sec:digsig-prerequisities} 节。

RSA\_DS 外设支持运算子长度为 \begin{math}N = 32 \times x\end{math} (\begin{math}x \in \{ 1,2,3, \dots, 128\} \end{math}) 的 RSA 签名运算 \begin{math}Z = X^{Y} \bmod M\end{math}，需要满足 RSA 计算的运算子长度要求，即 \begin{math}Z\end{math}、
\begin{math}X\end{math}、
\begin{math}Y\end{math}、
\begin{math}M\end{math} 和
\begin{math}\overline{r}\end{math}
的位宽必须相同，但必须为这 128 种中的其中一种，
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{./13-DIGSIG/figures/13-DIGSIG-flow__CN}
...
\vspace{-2.5em}
\begin{enumerate}
\item 对于多次签名计算，软件准备工作（图 \ref{fig:digsig-flow} 左侧）只需要进行一次，但对于每一次签名计算，硬件运算（图 1-1 右侧）都需要重新进行。
\end{enumerate}
\end{tiplisting}
...
\begin{itemize}
    ...
    \item {\bfseries 步骤 1}：按照第 \ref{sec:digsig-private-key-operands} 节所述准备好 RSA 私钥运算子 \begin{math}Y\end{math} 和 \begin{math}M\end{math}，它们应符合运算子的长度要求。
          记 \begin{math}[L]_{32} = \frac{N}{32}-1 \end{math}（比如，对于 RSA 4096，\begin{math}[L]_{32}\end{math} == [0x80-1]\begin{math}_{32}\end{math}）。还需要准备好 \begin{math}[HMAC\_KEY]_{256}\end{math}，并计算出 \begin{math}[RSA\_DS\_KEY]_{256}\end{math}， 即 \begin{math}RSA\_DS\_KEY\end{math} = HMAC-SHA256(\begin{math}[HMAC\_KEY]_{256}\end{math}, \begin{math}1^{256}\end{math})。 另外还需要引入一个随机的 \begin{math}[IV]_{128}\end{math}， 它需要符合 AES-CBC 块加密算法的要求。有关 AES 更多信息，请参考章节 \ref{mod:aes} \textit{\nameref{mod:aes}}。
    ...
    \item {\bfseries 步骤 5}：构造 \begin{math}[P]_{12672}\end{math} = (
            \begin{math}
                ...
                [L]_{32} ||
                [\beta]_{64})
            \end{math}，其中 \begin{math}[\beta]_{64} \end{math} 是符合 PKCS\#7 封装方式的追加码， 由 8 个 字节码 0x80 组成的一个 64 位的位串 $[$0x0808080808080808$]_{64}$，目的在于使 \begin{math}P\end{math} 的长度为 128 位的整数倍。

    \item {\bfseries 步骤 6}：
            计算密文形式的私钥参数 \begin{math}C\end{math} = \begin{math}[C]_{12672}\end{math} =
            AES-CBC-ENC (\begin{math}[P]_{12672}\end{math}, \begin{math}[RSA\_DS\_KEY]_{256}\end{math}, \begin{math}[IV]_{128}\end{math})，长度为 1584 字节。
...
\begin{enumerate}
    ...
    \item {\bfseries 将 \begin{math}X\end{math} 写入存储器 \hyperref[DSXMEM]{DS\_X\_MEM}}：将 \begin{math}X_{i}\end{math}
        ...
        写入存储器 \hyperref[DSXMEM]{DS\_X\_MEM}，容量为 128 个字 (word)，其中 \begin{math} n = \frac{N}{32} \end{math} 。
    ...
    \begin{itemize}
        ...
        \item 如果返回值为 3，则说明填充填充校验失败，且 MD 校验失败。 这种情况说明有致命错误发生。\begin{math}Z\end{math} 结果值全零无效。跳至步骤 \ref{Exit}。
...
请注意，这里的起始地址和结束地址都是相对于 RSA 数字签名算法 (RSA\_DS) 基地址的地址偏移量（相对地址）。请参阅章节 \ref{mod:sysmem} \textit{\nameref{mod:sysmem}} 中的表 \ref{tab:sysmem-base-address} 获取基地址。

\subfile{\modulefiles/13-DIGSIG-mem-summ__CN}
...
\subfile{\modulefiles/13-DIGSIG-reg-summ__CN}
...
\subfile{\modulefiles/13-DIGSIG-reg__CN}
