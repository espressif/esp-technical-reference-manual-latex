...
Table \ref{table:i2s-signal-description} provides a detailed description of I2S\regindex{n} signals.


\begin{table}[H]
    \centering
    \caption{I2S\regindex{n} Signal Description}
    \label{table:i2s-signal-description}
...
The channel data will be stored as the data to be input in order from high to low. The data bit order in each channel is controlled by  \hyperref[fielddesc:I2STXBITORDER]{I2S\_TX\_BIT\_ORDER}:
\begin{itemize}
\item 0: Not reverse the valid data bit order;
\item 1: Reverse the valid data bit order.
\end{itemize}
At this point, the data format control is complete. The data after format control will be sent sequentially from high to low. Figure  \ref{Figure:i2s_data_mode_control} shows a complete process of TX data format control.
...
\begin{enumerate}
    ...
    \item Configure signal pins according to Table  \ref{table:i2s-signal-description}.
