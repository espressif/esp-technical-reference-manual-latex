...
1：高比例 \\
(RO \textcolor{red}{| R/W})
\label{fielddesc:TWAISYNCJUMPWIDTH}\item [TWAI\_SYNC\_JUMP\_WIDTH] 配置同步跳宽 (SJW)，范围为 1 \verb+~+ 4 个时间定额。（RO \textcolor{red}{| R/W}）
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:TWAIRXINTST}\item [TWAI\_RX\_INT\_ST] \hyperref[other:twai-para-rxi]{TWAI\_RX\_INT} 的屏蔽中断状态。(RO)
\label{fielddesc:TWAITXINTST}\item [TWAI\_TX\_INT\_ST] \hyperref[other:twai-para-txi]{TWAI\_TX\_INT} 的屏蔽中断状态。(RO)
\label{fielddesc:TWAIERRWARNINTST}\item [TWAI\_ERR\_WARN\_INT\_ST] \hyperref[other:twai-para-ewi]{TWAI\_ERR\_WARN\_INT} 的屏蔽中断状态。(RO)
\label{fielddesc:TWAIOVERRUNINTST}\item [TWAI\_OVERRUN\_INT\_ST] \hyperref[other:twai-para-doi]{TWAI\_OVERRUN\_INT} 的屏蔽中断状态。(RO)
\label{fielddesc:TWAIERRPASSIVEINTST}\item [TWAI\_ERR\_PASSIVE\_INT\_ST] \hyperref[other:twai-para-epi]{TWAI\_ERR\_PASSIVE\_INT} 的屏蔽中断状态。(RO)
\label{fielddesc:TWAIARBLOSTINTST}\item [TWAI\_ARB\_LOST\_INT\_ST] \hyperref[other:twai-para-ali]{TWAI\_ARB\_LOST\_INT} 的屏蔽中断状态。(RO)
\label{fielddesc:TWAIBUSERRINTST}\item [TWAI\_BUS\_ERR\_INT\_ST] \hyperref[other:twai-para-bei]{TWAI\_BUS\_ERR\_INT} 的屏蔽中断状态。(RO)
\label{fielddesc:TWAIBUSSTATEINTST}\item [TWAI\_BUS\_STATE\_INT\_ST] \hyperref[other:twai-para-bisi]{TWAI\_BUS\_STATE\_INT} 的屏蔽中断状态。(RO)
\end{reglist}\end{regdesc}
\end{register}
