...
该部分介绍构成芯片系统的基本要素，包括 ESP-RISC-V CPU、RISC-V 追踪编码器、通用直接内存访问 (GDMA) 控制器等。
...
该部分描述系统的存储器组织结构，例如 RAM、ROM、eFuse 和外部存储器的组织和映射，为理解与存储器相关的子系统提供了框架。
...
该部分介绍芯片的系统级功能，包括系统启动、时钟、GPIO、中断处理、事件任务管理、低功耗管理、定时器、看门狗、权限控制、系统寄存器和辅助调试。
...
\subfileinclude{13-SYSTIMER__CN}
\subfileinclude{14-TIMG__CN}
\subfileinclude{15-WDT__CN}
\subfileinclude{16-PERMCTRL__CN}
\subfileinclude{17-SYSREG__CN}
\subfileinclude{18-DBGASSIST__CN}
\subfileinclude{40-PSDET__CN}
\part{加密和安全组件}

该部分专门介绍芯片的安全功能，包括 AES、ECC、HMAC 等加密加速器，数字签名、随机数生成、加密/解密算法等安全特性，展示了芯片在加密和安全数据处理方面的性能。

\subfileinclude{19-AES__CN}
\subfileinclude{20-ECC__CN}
\subfileinclude{21-HMAC__CN}
\subfileinclude{22-RSA__CN}
\subfileinclude{23-SHA__CN}
\subfileinclude{24-DIGSIG__CN}
\subfileinclude{25-ECDSA__CN}
\subfileinclude{26-EXTMEMENCR__CN}
\subfileinclude{27-RNG__CN}
...
\subfileinclude{28-UART__CN}
\subfileinclude{29-SPI__CN}
\subfileinclude{30-I2C__CN}
\subfileinclude{31-I2S__CN}
\subfileinclude{32-PCNT__CN}
\subfileinclude{33-USBSERIALJTAG__CN}
\subfileinclude{34-TWAI__CN}
\subfileinclude{35-LEDC__CN}
\subfileinclude{36-MCPWM__CN}
\subfileinclude{37-RMT__CN}
\subfileinclude{38-PARLIO__CN}
...
\subfileinclude{39-TMPSEN__CN}
\subfileinclude{40-ADC__CN}
