...
\chipname{} contains one TWAI controller. The controller can be connected to a TWAI bus via an external transceiver. The TWAI controller contains numerous advanced features and can be utilized in a wide range of use cases, such as automotive products, industrial automation controls, building automation, etc.

\section{Features}
The TWAI controller on \chipname{} supports the following features:
...
\begin{itemize}
    \item Standard Frame Format (SFF) that uses an 11-bit identifier
...
\begin{figure}[ht!]
    \centering
    \fbox{\includegraphics[width=0.8\textwidth]{\modulefiles/figures/34-TWAI-bit-fields__EN}}
...
\input{\modulefiles/tables/34-TWAI-data-frames-and-remote-frames__EN}
...
\begin{figure}[ht]
    \centering
    \fbox{\includegraphics[width=0.8\textwidth]{\modulefiles/figures/34-TWAI-fields-of-an-error-frame__EN}}
    \caption{Fields of an Error Frame}
    \label{fig:twai-fields-of-an-error-frame}
\end{figure}

\input{\modulefiles/tables/34-TWAI-error-frame__EN}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.85\textwidth]{\modulefiles/figures/34-TWAI-fields-of-an-overload-frame__EN}}
    \caption{Fields of an Overload Frame}
    \label{fig:twai-fields-of-an-overload-frame}
\end{figure}

\input{\modulefiles/tables/34-TWAI-overload-frame__EN}
...
\begin{itemize}
        ...
        \item For case 1, a maximum of two overload frames may be generated.
\end{itemize}
...
\begin{figure}[ht]
    \centering
    \fbox{\includegraphics[width=0.7\textwidth]{\modulefiles/figures/34-TWAI-fields-within-an-interframe-space__EN}}
    \caption{The Fields within an Interframe Space}
    \label{fig:twai-fields-within-interframe-space}
\end{figure}
\input{\modulefiles/tables/34-TWAI-interframe-space__EN}
...
\begin{figure}[ht]
    \centering
    \fbox{\includegraphics[width=0.55\textwidth]{\modulefiles/figures/34-TWAI-bit-timing__EN}}
    \caption{Layout of a Bit}
    \label{fig:twai-bit-timing}
\end{figure}

\input{\modulefiles/tables/34-TWAI-seg-of-a-nominal-bit-time__EN}
...
\begin{figure}[ht]
    \centering
    \fbox{\includegraphics[width=0.7\textwidth]{\modulefiles/figures/34-TWAI-overview.eps}}
...
The TWAI controller only works in the CORE clock domain, where RC\_FAST\_CLK or crystal clock XTAL\_CLK can be selected as the clock source. Users may configure \hyperref[regdesc:PCRTWAI0FUNCCLKCONFREG]{PCR\_TWAI0\_FUNC\_CLK\_CONF\_REG} to select the clock source.
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.4\textwidth]{\modulefiles/figures/34-TWAI-clk-gen.png}}
    \caption{TWAI Clock Generation}
    \label{fig:twai-clkgen-diagram}
\end{figure}
...
The configuration register stores various configuration items for the TWAI controller such as bit rates, Operation mode, Acceptance Filter, etc. Configuration registers can only be modified whilst the TWAI controller is in Reset mode (See Section \ref{sec:twai-modes}).
...
\begin{itemize}
    ...
    \item {\bfseries Self-Test mode:} Self-test mode is similar to Normal mode, but the TWAI controller will consider the transmission of a data or remote frame successful and not generate an ACK error even if it was not acknowledged. This mode is commonly used during the self-test of a TWAI controller.
...
The \chipname{} TWAI controller provides eight interrupts, each represented by a single bit in \hyperref[regdesc:TWAIINTSTREG]{TWAI\_INT\_ST\_REG}. For a particular interrupt to be triggered, the corresponding enable bit in \hyperref[regdesc:TWAIINTENAREG]{TWAI\_INT\_ENA\_REG} must be set.
...
\begin{itemize}
    \item A message transmission has been completed successfully, i.e., acknowledged without any errors. Any failed messages will automatically be resent.
    \item A single shot transmission has been completed (successfully or unsuccessfully, indicated by the \hyperref[fielddesc:TWAITXCOMPLETE]{TWAI\_TX\_COMPLETE} bit).
...
\label{other:twai-para-ewi}
The Error Warning Interrupt (EWI) is triggered whenever there is a change to the \hyperref[fielddesc:TWAIERRST]{TWAI\_ERR\_ST} and \hyperref[fielddesc:TWAIBUSOFFST]{TWAI\_BUS\_OFF\_ST} bits of \hyperref[regdesc:TWAISTATUSREG]{TWAI\_STATUS\_REG} (i.e., transition from 0 to 1 or vice versa). Thus, an EWI could indicate one of the following events, depending on the values of \hyperref[fielddesc:TWAIERRST]{TWAI\_ERR\_ST} and \hyperref[fielddesc:TWAIBUSOFFST]{TWAI\_BUS\_OFF\_ST} at the moment when the EWI is triggered.
...
\subsubsection{Error Passive Interrupt (EPI)}
\label{other:twai-para-epi}
...
\label{other:twai-para-ali}
The Arbitration Lost Interrupt (ALI) is triggered whenever the TWAI controller is attempting to transmit a message and loses arbitration. The bit position where the TWAI controller lost arbitration is automatically recorded in the Arbitration Lost Capture register (\hyperref[regdesc:TWAIARBLOSTCAPREG]{TWAI\_ARB\_LOST\_CAP\_REG}). When the ALI occurs again, the Arbitration Lost Capture register will no longer record a new bit location until it is cleared (via CPU reading this register).

\subsubsection{Bus Error Interrupt (BEI)}
\label{other:twai-para-bei}
...
\input{\modulefiles/tables/34-TWAI-buffer-layout__EN}
...
\begin{figure}[ht]
    \centering
    \fbox{\includegraphics[width=1.0\textwidth]{\modulefiles/figures/34-TWAI-acceptance-filter}}
...
Single Filter mode is enabled by setting the \hyperref[fielddesc:TWAIRXFILTERMODE]{TWAI\_RX\_FILTER\_MODE} bit to 1. This will cause the 32-bit code and mask values to define a single filter. The single filter can filter the following bits of data or remote frames:
...
\begin{figure}[ht]
    \centering
    \fbox{\includegraphics[width=0.8\textwidth]{\modulefiles/figures/34-TWAI-single-filter-mode__EN}}
...
Dual Filter mode is enabled by clearing the \hyperref[fielddesc:TWAIRXFILTERMODE]{TWAI\_RX\_FILTER\_MODE} bit to 0. This will cause the 32-bit code and mask values to define two separate filters referred to as filter 1 or filter 2. Under Dual Filter mode, a message will be accepted if it is accepted by one of the two filters.
...
\begin{figure}[ht!]
    \centering
    \fbox{\includegraphics[width=0.6\textwidth]{\modulefiles/figures/34-TWAI-dual-filter-mode__CN}}
...
The current error state of the TWAI controller is indicated via a combination of the following values and status bits: TEC, REC, \hyperref[fielddesc:TWAIERRST]{TWAI\_ERR\_ST}, and \hyperref[fielddesc:TWAIBUSOFFST]{TWAI\_BUS\_OFF\_ST}. Certain changes to these values and bits will also trigger interrupts, so that users are notified of error state transitions (see section \ref{sec:twai-interrupt-management}). The following figure \ref{fig:twai-error-state-transition} shows the relation between the error states, values and bits, and error state related interrupts.

\begin{figure}[ht!]
    \centering
    \fbox{\includegraphics[width=0.8\textwidth]{\modulefiles/figures/34-TWAI-error-state-transition__EN}}
...
\input{\modulefiles/tables/34-TWAI-bit-info-seg0-to-seg4__EN}
...
\begin{figure}[ht]
    \centering
    \fbox{\includegraphics[width=0.9\textwidth]{\modulefiles/figures/34-TWAI-arbitration-lost-position__EN}}
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item Configure the \hyperref[regdesc:TWAIHWSTANDBYCNTREG]{TWAI\_HW\_STANDB\_CNT\_REG} register. This register indicates the time required before hardware triggers the standby signal after entering idle status, in which the value indicates the number of cycles of the TWAI controller operating clock (32 MHz by default).
    \end{enumerate}
...
\subfile{\modulefiles/34-TWAI-reg-summ__EN}
...
\subfile{\modulefiles/34-TWAI-reg__EN}
