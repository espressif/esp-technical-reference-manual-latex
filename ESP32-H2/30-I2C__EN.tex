...
\begin{itemize}
    ...
    \item Continuous data transfer by pulling SCL low in slave mode
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{\modulefiles/figures/30-I2C-master-architecture}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{\modulefiles/figures/30-I2C-slave-architecture}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{\modulefiles/figures/30-I2C-timing-definition}
...
\begin{table}[H]
    \centering
    \fbox{\includegraphics[width=0.9\textwidth]{\modulefiles/figures/30-I2C-timing-characteristic}}
...
You can choose the clock source for I2C\_SCLK from XTAL\_CLK or RC\_FAST\_CLK via \hyperref[fielddesc:PCRI2C0SCLKSEL]{PCR\_I2C\_SCLK\_SEL}:

\begin{itemize}
    \item Enable the clock source for I2C\_SCLK by configuring \hyperref[fielddesc:PCRI2C0SCLKEN]{PCR\_I2C\_SCLK\_EN} to 1.
    \item When \hyperref[fielddesc:PCRI2C0SCLKSEL]{PCR\_I2C\_SCLK\_SEL} is 0, the clock source is XTAL\_CLK. 
    \item When \hyperref[fielddesc:PCRI2C0SCLKSEL]{PCR\_I2C\_SCLK\_SEL} is 1, the clock source is RC\_FAST\_CLK. 
\end{itemize}

The clock source then passes through a fractional divider to generate I2C\_SCLK according to the following equation:

\[
Divisor = PCR\_I2C\_SCLK\_DIV\_ NUM + 1+\frac{ \hyperref[fielddesc:PCRI2C0SCLKDIVA]{PCR\_I2C\_SCLK\_DIV\_A}}{\hyperref[fielddesc:PCRI2C0SCLKDIVB]{PCR\_I2C\_SCLK\_DIV\_B}}
\]
...
The slave can choose to hold the SCL line low for clock stretching when one of the following four events occurs:
...
\item RAM being full: RX RAM of the slave is full. Note that when the slave receives less data than the FIFO depth, which is 32 bytes in \chipname{} I2C, it is not necessary to enable clock stretching; when the slave receives FIFO depth bytes or more, you may interrupt data transmission to wrapped around RAM via the FIFO threshold, or enable clock stretching for more time to process data. When clock stretching is enabled, \hyperref[fielddesc:I2CRXFULLACKLEVEL]{I2C\_RX\_FULL\_ACK\_LEVEL} must be cleared, otherwise, there will be unpredictable consequences.
...
When clock stretching occurs, the cause of stretching can be read from the \hyperref[fielddesc:I2CSTRETCHCAUSE]{I2C\_STRETCH\_CAUSE} bit. Clock stretching can be cleared by setting the \hyperref[fielddesc:I2CSLAVESCLSTRETCHCLR]{I2C\_SLAVE\_SCL\_STRETCH\_CLR} bit.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{\modulefiles/figures/30-I2C-bus-timing.eps}
    \caption{I2C Timing Diagram}
    \label{fig:i2c-bus-timing}
\end{figure}

Figure \ref{fig:i2c-bus-timing} shows the timing diagram of an I2C master. This figure also specifies registers used to configure the START bit, STOP bit, data hold time, data sample time, waiting time on the rising SCL edge, etc. Timing parameters (please refer to Table \ref{fig:i2c-timing-param}) are calculated as follows in I2C\_SCLK clock cycles: 
...
The I2C controller has three types of timeout control, namely timeout control for SCL\_FSM, SCL\_MAIN\_FSM, and the SCL line. The first two are always enabled, while the third is configurable.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{\modulefiles/figures/30-I2C-cmd-structure}
...
\begin{itemize}
    ...
    \item RSTART: op\_code = 6. The I2C controller sends a START bit or an RSTART bit defined by the I2C protocol.
...
Both TX RAM and RX RAM are 32 × 8 bits, and can be accessed in FIFO or non-FIFO mode. If the \hyperref[fielddesc:I2CNONFIFOEN]{I2C\_NONFIFO\_EN} bit is cleared, both RAMs are accessed in FIFO mode; if the \hyperref[fielddesc:I2CNONFIFOEN]{I2C\_NONFIFO\_EN} bit is set, both RAMs are accessed in non-FIFO mode.
...
Assume the slave address is SLV\_ADDR. In 7-bit addressing mode, the slave address is SLV\_ADDR[6:0]; in 10-bit addressing mode, the slave address is SLV\_ADDR[9:0].
...
To start the I2C controller in master mode, after configuring the controller to master mode and command registers, write 1 to \hyperref[fielddesc:I2CTRANSSTART]{I2C\_TRANS\_START} in order to let the master starts to parse and execute command sequences. The master always executes a command sequence starting from the command register 0 to a STOP or an END. To execute another command sequence starting from command register 0, refresh commands by writing 1 again to \hyperref[fielddesc:I2CTRANSSTART]{I2C\_TRANS\_START}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{\modulefiles/figures/30-I2C-MWS7}
...
\begin{enumerate}
    ...
    \item Execute an RSTART command by sending a START bit.
...
\item Write the address of I2C$_\text{slave}$ to \hyperref[fielddesc:I2CSLAVEADDR]{I2C\_SLAVE\_ADDR} (slave) in the \hyperref[regdesc:I2CSLAVEADDRREG]{I2C\_SLAVE\_ADDR\_REG} (slave) register.
\item Write 1 to \hyperref[fielddesc:I2CCONFUPGATE]{I2C\_CONF\_UPGATE} (master) and \hyperref[fielddesc:I2CCONFUPGATE]{I2C\_CONF\_UPGATE} (slave) to synchronize registers.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{\modulefiles/figures/30-I2C-MWS10}
...
\item Write 1 to \hyperref[fielddesc:I2CCONFUPGATE]{I2C\_CONF\_UPGATE} (master) and \hyperref[fielddesc:I2CCONFUPGATE]{I2C\_CONF\_UPGATE} (slave) to synchronize registers.
\item Write 1 to \hyperref[fielddesc:I2CTRANSSTART]{I2C\_TRANS\_START} (master) and \hyperref[fielddesc:I2CTRANSSTART]{I2C\_TRANS\_START} (slave) to start transfer.
\item I2C$_\text{slave}$ compares the slave address sent by I2C$_\text{master}$ with its own address in \hyperref[fielddesc:I2CSLAVEADDR]{I2C\_SLAVE\_ADDR} (slave). When ack\_check\_en (master) in I2C$_\text{master}$'s WRITE command is 1, I2C$_\text{master}$ checks ACK value each time it sends a byte. When ack\_check\_en (master) is 0, I2C$_\text{master}$ does not check the ACK value and takes I2C$_\text{slave}$ as the matching slave by default.
\begin{itemize}
\item Match: If the received ACK value matches ack\_exp (master) (the expected ACK value), I2C$_\text{master}$ continues data transfer.
\item Not match: If the received ACK value does not match ack\_exp, I2C$_\text{master}$ generates an I2C\_NACK\_INT (master) interrupt and stops data transfer.
\end{itemize}
...
\item If the data to be sent is larger than TX FIFO depth, TX RAM of I2C$_\text{master}$ may wrap around in FIFO mode. For details, please refer to Section \ref{subsubsec:i2c-txrx}. 
\item If the data to be received is larger than RX FIFO depth, RX RAM of I2C$_\text{slave}$ may wrap around in FIFO mode. For details, please refer to Section \ref{subsubsec:i2c-txrx}. 

If the data to be received is larger than RX FIFO depth, the other way is to enable clock stretching by setting \hyperref[fielddesc:I2CSLAVESCLSTRETCHEN]{I2C\_SLAVE\_SCL\_STRETCH\_EN} (slave), and clearing \hyperref[fielddesc:I2CRXFULLACKLEVEL]{I2C\_RX\_FULL\_ACK\_LEVEL} to 0. When RX RAM is full, an \hyperref[int:i2c-slave-stretch]{I2C\_SLAVE\_STRETCH\_INT} (slave) interrupt is generated. In this way, I2C$_\text{slave}$ can hold SCL low, in exchange for more time to read data. After the software has finished reading, you can set \hyperref[fielddesc:I2CSLAVESTRETCHINTCLR]{I2C\_SLAVE\_STRETCH\_INT\_CLR} (slave) to 1 to clear interrupt, and set \hyperref[fielddesc:I2CSLAVESCLSTRETCHCLR]{I2C\_SLAVE\_SCL\_STRETCH\_CLR} (slave) to release the SCL line.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{\modulefiles/figures/30-I2C-MWS7-M}
...
The I2C slave put received byte0 \verb+~+ byte(N-1) into its RAM in an order starting from addrM. The RAM is overwritten every 32 bytes.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{\modulefiles/figures/30-I2C-MWS-END}    \caption{I2C$_\text{master}$ Writing to I2C$_\text{slave}$ with a 7-bit Address in Multiple Sequences}
...
\end{longtable} 
\item Write 1 to \hyperref[fielddesc:I2CTRANSSTART]{I2C\_TRANS\_START} (master) to start transfer.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{\modulefiles/figures/30-I2C-MRS10}
    \caption{I2C$_\text{master}$ Reading I2C$_\text{slave}$ with a 10-bit Address}
    \label{fig:i2c-mrs10}
\end{figure}

Figure \ref{fig:i2c-mrs10} shows how I2C$_\text{master}$ reads data from an I2C slave using 10-bit addressing. Unlike 7-bit addressing, in 10-bit addressing the WRITE command of the I2C$_\text{master}$ is formed from two bytes, and correspondingly TX RAM of this master stores a 10-bit address of two bytes. The $R/\overline W$ bit in the first byte is 0, which indicates a WRITE operation. After an RSTART condition, I2C$_\text{master}$ sends the first byte of address again to read data from I2C$_\text{slave}$, but the $R/\overline W$ bit is 1, which indicates a READ operation. The two address bytes can be configured as described in Section \ref{sub:i2c-app-mws10}.
...
\item I2C$_\text{master}$ sends an RSTART and the third byte in TX RAM, which is ((0x78 | \hyperref[fielddesc:I2CSLAVEADDR]{I2C\_SLAVE\_ADDR}[9:8])<<1) and a $R/\overline W$ bit that indicates READ.
...
\item Configure \hyperref[fielddesc:I2CSLAVEADDR]{I2C\_SLAVE\_ADDR} (slave) in the \hyperref[regdesc:I2CSLAVEADDRREG]{I2C\_SLAVE\_ADDR\_REG} (slave) register as I2C$_\text{slave}$'s 7-bit address, and set \hyperref[fielddesc:I2CADDR10BITEN]{I2C\_ADDR\_10BIT\_EN} (slave) to 0 to enable 7-bit addressing.
...
\item I2C$_\text{master}$ sends an RSTART and the third byte in TX RAM, which is ((0x78 | \hyperref[fielddesc:I2CSLAVEADDR]{I2C\_SLAVE\_ADDR}[6:0])<<1) and an R bit.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{\modulefiles/figures/30-I2C-MRS-END}
...
\item If the last command is an END, then repeat step 16 and proceed to the next steps.
\item Update I2C$_\text{master}$'s command registers.
\begin{longtable}{ | p{4cm} | p{2cm} | p{2cm} | p{2cm} |p{2cm} | p{2cm} |}
\hline\rowcolor{lightgray}
Command registers of I2C$_\text{master}$& op\_code & ack\_value&ack\_exp&ack\_check\_en&byte\_num  \\ \hline
...
\begin{itemize}
    ...
    \item \label{int:i2c-general-call}I2C\_GENERAL\_CALL\_INT: Triggered when the slave receives general call address.
...
\subfile{\modulefiles/30-I2C-ext-reg-summ__EN.tex}
...
\subfile{\modulefiles/30-I2C-ext-reg__EN.tex}
