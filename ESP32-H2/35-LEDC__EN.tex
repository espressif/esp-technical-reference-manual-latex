...
Timers in LED PWM Controller choose their common clock source from one of the following clock signals: PLL\_F96M\_CLK, RC\_FAST\_CLK, and XTAL\_CLK. The procedure for selecting a clock source signal for LEDC\_CLK\regindex{x} is described below:

\begin{itemize}
    \item PLL\_F96M\_CLK: Set \hyperref[fielddesc:LEDCSCLKSEL]{LEDC\_SCLK\_SEL}[1:0] to 1
...
If \hyperref[fielddesc:LEDCSCLKSEL]{LEDC\_SCLK\_SEL}[1:0] is set to 0 (an invalid value), LED PWM Controller cannot perform any function due to no clock source.
...
The divisor LEDC\_CLK\_DIV can be non-integer values, and it is configured according to the following equation.
...
When the fractional part \textit{B} is 0, LEDC\_CLK\_DIV is an integer (i.e., an integer prescaler). In other words, a ref\_pulse\regindex{x} clock pulse is generated after every \textit{A} LEDC\_CLK\regindex{x} clock pulses.

However, when \textit{B} is not 0, LEDC\_CLK\_DIV becomes a non-integer. The clock divider implements non-integer frequency division by generating a ref\_pulse\regindex{x} clock pulse after every \textit{A} and (\textit{A}+1) LEDC\_CLK\regindex{x} clock pulses alternately. In this way, the average frequency of ref\_pulse\regindex{x} clock pulse will be the desired frequency (i.e., the non-integer divided frequency). For every 256 ref\_pulse\regindex{x} clock pulses:
...
Figure \ref{fig:ledc-divider} illustrates the relation between LEDC\_CLK\regindex{x} clock pulses and ref\_pulse\regindex{x} clock pulses when LEDC\_CLK\_DIV is a non-integer.
...
Every time the counter overflows, it can trigger the \hyperref[int:ledc-timer-ovf]{LEDC\_TIMER\regindex{x}\_OVF\_INT} interrupt (generated automatically by hardware without configuration). It can also be configured to trigger \hyperref[int:ledc-ovf-cnt]{LEDC\_OVF\_CNT\_CH\regindex{n}\_INT} interrupt after overflowing $\textrm{\hyperref[fielddesc:LEDCOVFNUMCHN]{LEDC\_OVF\_NUM\_CH\regindex{n}} + 1}$ times. To configure \hyperref[int:ledc-ovf-cnt]{LEDC\_OVF\_CNT\_CH\regindex{n}\_INT} interrupt, please:
 ...
 \begin{enumerate}
    ...
    \item Set \hyperref[fielddesc:LEDCOVFCNTENCHN]{LEDC\_OVF\_CNT\_EN\_CH\regindex{n}} to enable the overflow counter
    \item Configure
    ...
    \item Set \hyperref[fielddesc:LEDCOVFCNTCHNINTENA]{LEDC\_OVF\_CNT\_CH\regindex{n}\_INT\_ENA} to enable the overflow interrupt
    \item Set \hyperref[fielddesc:LEDCTIMERXDUTYRES]{LEDC\_TIMER\regindex{x}\_DUTY\_RES} to enable the timer and wait for a \hyperref[int:ledc-ovf-cnt]{LEDC\_OVF\_CNT\_CH\regindex{n}\_INT} interrupt
\end{enumerate}
...
Figure \ref{fig:ledc-fixed-duty} illustrates how Hpoint\regindex{n} and Lpoint\regindex{n} are used to generate the PWM output signal with a fixed duty cycle.

\begin{figure}[ht]
    \centering
    \fbox{\includegraphics[width=1\textwidth]{\modulefiles/figures/35-LEDC-fixed-duty-en}}
...
\subfile{\modulefiles/35-LEDC-reg-summ__EN}
...
\subfile{\modulefiles/35-LEDC-reg__EN}
