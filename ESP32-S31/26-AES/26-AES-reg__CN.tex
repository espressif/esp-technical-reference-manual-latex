...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:AESMODE}\item [AES\_MODE] 配置 AES 加速器的密钥长度和加解密方向。\\
0：AES-128 加密\\
1：保留\\
2：AES-256 加密\\
3：保留\\
...
7：保留\\
(R/W)
\label{fielddesc:AESUSEHARDWAREKEY}\item [AES\_USE\_HARDWARE\_KEY] 配置 AES 加速器使用软件密钥或硬件密钥。\\
0：软件密钥\\
1：硬件密钥\\ (R/W)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:AESTRIGGER}\item [AES\_TRIGGER] 配置是否启动 AES 运算。\\
0：无效果\\
1：启动\\
(WT)
\end{reglist}\end{regdesc}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\item [AES\_CONTINUE\_REG] 配置是否继续 AES 运算。\\
0：无效果\\
1：继续 \\ (WO)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:AESRXRESET}\item [AES\_RX\_RESET] 配置在 DMA-AES 工作模式下是否复位 RX\_FIFO。\\
0：无效过\\
1：复位\\
(WT)
\end{reglist}\end{regdesc}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:AESTXRESET}\item [AES\_TX\_RESET] 配置在 DMA-AES 工作模式下是否复位 TX\_FIFO。\\
0：无效过\\
1：复位\\
(WT)
\end{reglist}\end{regdesc}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:AESPSEUDOEN}\item [AES\_PSEUDO\_EN] 配置是否启用伪轮函数。\\
0：不启用\\
1：启用\\
(R/W)
\label{fielddesc:AESPSEUDOBASE}\item [AES\_PSEUDO\_BASE] 配置基本轮数。(R/W)
\label{fielddesc:AESPSEUDOINC}\item [AES\_PSEUDO\_INC] 配置递增轮数。(R/W)
\label{fielddesc:AESPSEUDORNGCNT}\item [AES\_PSEUDO\_RNG\_CNT] 配置伪密钥的更新频率。(R/W)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:AESFREE}\item [AES\_FREE] 配置是否允许释放 AES（即使 AES 正在运行中），以便其他需要使用 AES 的模块可以占用 AES 资源。\\
0：不可释放 AES\\
1：释放 AES\\
(R/W)
\end{reglist}\end{regdesc}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:AESDATE}\item [AES\_DATE] 版本控制寄存器。(R/W)
\end{reglist}\end{regdesc}
\end{register}
