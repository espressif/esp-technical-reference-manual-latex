...
The SHA accelerator has the following features:
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item SHA-512/{\it{t}}
    \end{itemize}
    \item The following hash algorithms introduced in   \href{http://www.gmbz.org.cn/file/2018-01-08/6db059ec-61e1-4ff9-b112-ae15471ad6b5.PDF}{GM/T 0004-2012}
    \begin{itemize}
        \item SM3
    \end{itemize}
...
The SHA accelerator is activated by setting the \hyperref[fielddesc:HPSYSCLKRSTCRYPTOSHACLKEN]{HP\_SYS\_CLKRST\_CRYPTO\_SHA\_CLK\_EN} register field and clearing the \hyperref[fielddesc:HPSYSCLKRSTSHARSTEN]{HP\_SYS\_CLKRST\_SHA\_RST\_EN} register field. 
...
Users can choose hash algorithms by configuring the \hyperref[regdesc:SHAMODEREG]{SHA\_MODE\_REG} register. For details, please see Table \ref{tab:sha-mode-summ}. Support for the Chinese cryptographic standard (SM3) can be disabled by \hyperref[fielddesc:EFUSEDISSMCRYPT]{EFUSE\_DIS\_SM\_CRYPT}.
...
\vspace{-0.5em}
    ...
    \chipname{}'s \nameref{mod:rsads} and \nameref{mod:hmac} modules also call the SHA accelerator when working. By default, \hyperref[fielddesc:SHAFREE]{SHA\_FREE} (with value 1) would allow \nameref{mod:rsads} and \nameref{mod:hmac} modules to immediately reset any current SHA operation and use SHA for its operations. Users should set \hyperref[fielddesc:SHAFREE]{SHA\_FREE} to 0 in order to prevent this from happening.
...
\begin{itemize}
    \item \textbf{SHA-1}, \textbf{SHA-224}, \textbf{SHA-256}, and \textbf{SM-3}
        \begin{enumerate}
...
\begin{itemize}
    \item For \textbf{SHA-1}, \textbf{SHA-224}, \textbf{SHA-256}, and \textbf{SM-3}: 
...
Before hash task begins for each of the secure hash algorithms, the initial Hash value H$^{(0)}$ must be set based on different algorithms, among which the SHA-1, SHA-224, SHA-256, SHA-384, SHA-512, SHA-512/224, SHA-512/256, and SM3 algorithms use the initial Hash values (constant C) stored in the hardware. 
...
\vspace{-2.5em}
\begin{enumerate}
\item In this step, the software can also write the next message block, if any, to the \hyperref[regdesc:SHAMNREG]{SHA\_M\_\regindex{n}\_REG} registers, while the hardware is performing the SHA calculation, to reduce processing time.
...
As mentioned above, \chipname{} SHA accelerator supports \textbf{``interleaving" calculation under the Typical SHA working mode}. The process for interleaved calculation is described below.
...
\begin{enumerate}
    ...
    \item Start the DMA-SHA calculation.
        \begin{itemize}
            \item If the current DMA-SHA calculation follows a previous calculation, first write the message digest from the previous calculation to registers \hyperref[regdesc:SHAHNREG]{SHA\_H\_\regindex{n}\_REG}, then write 1 to register \hyperref[regdesc:SHADMACONTINUEREG]{SHA\_DMA\_CONTINUE\_REG} to start SHA accelerator;
...
\chipname{}'s SHA accelerator can generate the following interrupt signal(s) that will be sent to the \href{mod:intmtrx}{Interrupt Matrix}.
...
\hyperref[int:SHACALCDONEINT]{SHA\_CALC\_DONE\_INT}  &  Completion of an message digest calculation in DMA-SHA mode &  SHA\_INTR  \\ \hline
\end{longtable}
