...
0: Trigger an exception with mcause = 3 as described in the Privileged Architecture specification\\
1: Enter debug mode\\
(R/W)
...
(R/W)
\label{prv}\item [prv] Contains the privilege level the core is operating in when debug mode is entered. A debugger can change this value to change the core's privilege level when exiting debug mode. Only \textbf{0x3} (machine mode) and \textbf{0x0} (user mode) are supported. (R/W)
