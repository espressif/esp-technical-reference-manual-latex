...
\begin{itemize}
    \item Large-number modular exponentiation (maximum operand length: 4,096 bits) with two additional acceleration options to further increase the calculation speed
    \item Large-number modular multiplication (maximum operand length: 4,096 bits)
    \item Large-number multiplication (maximum operand length: 2,048 bits)
...
The RSA accelerator is activated in the \hyperref[regdesc:HPSYSCLKRSTCRYPTOCTRL0REG]{HP\_SYS\_CLKRST\_CRYPTO\_CTRL0\_REG} register by setting the \hyperref[fielddesc:HPSYSCLKRSTREGCRYPTORSACLKEN]{HP\_SYS\_CLKRST\_REG\_CRYPTO\_RSA\_CLK\_EN} bit to enable the clock and clearing the \hyperref[fielddesc:HPSYSCLKRSTREGCRYPTORSARSTEN]{HP\_SYS\_CLKRST\_REG\_CRYPTO\_RSA\_RST\_EN} bit to release the RSA accelerator from reset.
...
The \hyperref[fielddesc:RSAINTENA]{RSA\_INT\_ENA} field is used to control the interrupt triggered on completion of computation. Write 1 or 0 to this field to enable or disable the interrupt. By default, the interrupt function of the RSA accelerator is disabled.

\begin{importantlisting}
\vspace{-0.5em}

    \chipname{}'s \nameref{mod:rsads} module also calls the RSA accelerator when working.
    By default, if \hyperref[regdesc:RSAFREEREG]{RSA\_FREE\_REG} is 1, the \nameref{mod:rsads} can immediately abort any in-progress RSA operation and use the RSA accelerator to complete its operation. Set \hyperref[regdesc:RSAFREEREG]{RSA\_FREE\_REG} to 0 to prevent this.
\end{importantlisting}

\subsection{Definitions and Representations} \label{sec:rsa-definitions-representations}

\chipname{}'s RSA accelerator supports operands of different lengths \begin{math}N = 32 \times n\end{math}.
...
The RSA accelerator supports operands of length \begin{math}N = 32 \times n\end{math}, where \begin{math}n \in \{ 1,2,3, \dots, 128\} \end{math}. The bit lengths of arguments \begin{math}Z\end{math}, \begin{math}X\end{math}, \begin{math}Y\end{math}, \begin{math}M\end{math}, and \begin{math}\overline{r}\end{math}
...
\begin{enumerate}
    ...
    \item Wait for the completion of computation, which happens when the content of \hyperref[fielddesc:RSAQUERYIDLE]{RSA\_QUERY\_IDLE} becomes 1 or the RSA interrupt occurs, if enabled.
    ...
    \item If you have the interrupt enabled, write 1 to \hyperref[fielddesc:RSACLEARINTERRUPT]{RSA\_CLEAR\_INTERRUPT} to clear the interrupt.
...
Therefore, similar to the large-number modular exponentiation, two additional arguments are required -- \begin{math}\overline{r}\end{math} and
\begin{math}M^{\prime}\end{math}, which need to be calculated in advance by software.


The RSA accelerator supports large-number modular multiplication with operands of length \begin{math}N = 32 \times n\end{math}, where \begin{math}n \in \{ 1,2,3, \dots, 128\} \end{math}.
...
\begin{enumerate}
    ...
    \item Configure relevant configuration registers:
    ...
    \item Write
        ...
        \begin{math}Y_{i}\end{math},
        \begin{math}M_{i}\end{math}, and
        \begin{math}\overline{r}_{i}\end{math} for \begin{math}i \in \{ 0, 1, \dots, n-1 \} \end{math} to memory blocks \hyperref[regdesc:RSAmemory]{RSA\_X\_MEM}, \hyperref[regdesc:RSAmemory]{RSA\_Y\_MEM}, \hyperref[regdesc:RSAmemory]{RSA\_M\_MEM}, and \hyperref[regdesc:RSAmemory]{RSA\_Z\_MEM}, respectively. The capacity of each memory block is 128 words. Each word of each memory block can store one base-\begin{math}b\end{math} digit. The memory blocks use the little endian format for storage, i.e., the least significant digit of each number is in the lowest address.
...
Users can choose to use one or both of these options to further accelerate the computation. Note that, even when neither of these two options is configured, using the hardware RSA accelerator is still much faster than implementing the RSA algorithm in software.

To be more specific, when neither of these two options are configured for additional acceleration, the time required to calculate \begin{math}Z = X^{Y} \bmod M\end{math} is solely determined by the lengths of operands.  When either or both of these two options are configured for additional acceleration, the time required is also correlated with the 0 and 1 bit distribution in \begin{math}Y\end{math}.

To better illustrate how these two options work, first assume \begin{math}Y \end{math} is represented in binary as
...
Below is an example to demonstrate the performance of the RSA accelerator under different combinations of \hyperref[regdesc:RSASEARCHENABLEREG]{SEARCH} and \hyperref[regdesc:RSACONSTANTTIMEREG]{CONSTANT\_TIME} configuration. In this example:
\begin{itemize}
...
\subfile{\modulefiles/29-RSA-mem-summ__EN}
...
\subfile{\modulefiles/29-RSA-reg-summ__EN}
...
\subfile{\modulefiles/29-RSA-reg__EN}
