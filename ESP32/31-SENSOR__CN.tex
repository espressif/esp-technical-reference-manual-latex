...
\begin{figure}[!ht]
    \centering
    \fbox{\includegraphics[width=0.9\textwidth]{./31-SENSOR/figures/touch_fsm.eps}}
    \caption{FSM 的内部结构}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.7\textwidth]{./31-SENSOR/figures/saradc.eps}}
    \caption{SAR ADC 的概况}
...
\begin{figure}[!ht]
    \centering
    \fbox{\includegraphics[width=0.8\textwidth]{./31-SENSOR/figures/saradc_rtc_ctrl.eps}}
...
\begin{figure}[!ht]
    \centering
    \fbox{\includegraphics[width=0.8\textwidth]{./31-SENSOR/figures/dac.eps}}
    \caption{DAC 的功能概况}
...
\vspace{-2em}
\begin{itemize}
    \item 余弦波形生成器内部使用计数器控制波形周期，该计数器不对软件开放读写。若在发生器已使能时减小\\ \hyperref[regdesc:SENSSARDACCTRL1REG]{SENS\_SAR\_SW\_FSTEP}，且内部计数器当前值已大于新的 FSTEP，则必须等待计数器溢出后才能按新频率重新从 0 计数，期间波形时序可能不正确。
    \item 动态调整频率时，建议先清零 \hyperref[regdesc:SENSSARDACCTRL1REG]{SENS\_SW\_TONE\_EN} 禁用发生器，再写入新的 FSTEP，最后重新置位 \\\hyperref[regdesc:SENSSARDACCTRL1REG]{SENS\_SW\_TONE\_EN}，使计数器从 0 重新开始。
\end{itemize}
\end{tiplisting}

\begin{figure}[!ht]
    \centering
    \fbox{\includegraphics[width=0.8\textwidth]{./31-SENSOR/figures/dac2.eps}}
