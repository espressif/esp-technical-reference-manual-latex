...
\vspace{0.5em}
\begin{itemize}
    \item 章节 \ref{mod:ulp} \textit{\nameref{mod:ulp}}：更正表 \ref{table:adc inputs1} \textit{\nameref{table:adc inputs1}} 中 32K\_XP 和 32K\_XN 的 GPIO 编号 %DOC-15432
    \item 章节 \ref{mod:rmt} \textit{\nameref{mod:rmt}}：
        \begin{itemize}
            \item 新增中断 RMT\_CH\textcolor{red}{\it{n}}\_ERR\_INT
            \item 在章节 \ref{sec:rmt-reg-summ} \textit{\nameref{sec:rmt-reg-summ}} 中增加寄存器 \hyperref[regdesc:RMTCHnDATAREG]{RMT\_CH\textcolor{red}{\it{n}}DATA\_REG}、\hyperref[regdesc:RMTCHnSTATUSREG]{RMT\_CH\textcolor{red}{\it{n}}STATUS\_REG}、\hyperref[regdesc:RMTCHnADDRREG]{RMT\_CH\textcolor{red}{\it{n}}ADDR\_REG} 和 \hyperref[regdesc:RMTDATEREG]{RMT\_DATE\_REG}
            \item 更新寄存器描述，并将 RMT\_MEM\_ACCESS\_EN 更名为 RMT\_APB\_FIFO\_MASK
        \end{itemize}
    \item 章节 \ref{mod:sensor} \textit{\nameref{mod:sensor}}：补充 DAC 余弦波形生成器动态调整频率时的限制说明
\end{itemize}
\vspace{-1em}
\\\hline
2026.07 & v5.8 &
