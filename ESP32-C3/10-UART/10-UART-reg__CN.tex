...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:UARTAPBRXRADDR}\item [UART\_APB\_RX\_RADDR] 在软件通过 APB 总线读取 RX FIFO 数据时存储 RX FIFO 的偏移地址。UART0 为 0x200，UART1 为 0x280。 (RO)
\label{fielddesc:UARTRXWADDR}\item [UART\_RX\_WADDR] 在 Rx\_FIFO\_Ctrl  写 RX FIFO 时存储 RX FIFO 的偏移地址。 (RO)
\end{reglist}\end{regdesc}
\end{register}
