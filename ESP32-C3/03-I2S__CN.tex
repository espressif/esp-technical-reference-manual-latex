...
\end{figure}
图 \ref{fig:i2s-sys-diagram} 是 \chipname{} I2S 模块的结构框图。\chipname{} I2S 模块包含：
\begin{itemize}
...
图 \ref{fig:i2s-sys-diagram} 右侧为 I2S 模块的信号总线。RX 和 TX 模块的信号命名规则为：I2S\textcolor{red}{A}\_\textcolor{red}{B}\_\textcolor{red}{C}，例如 I2S\textcolor{red}{I}\_\textcolor{red}{BCK}\_\textcolor{red}{in}。其中：
...
与 Philips 标准相比，TDM Philips 标准支持更多的通道，见图 \ref{fig:i2s-tdm-philips-std-timing-diagram}。
\begin{figure}[H]
    \centering
...
与 MSB 对齐标准相比，TDM MSB 对齐标准支持更多的通道，见图 \ref{fig:i2s-tdm-msb-align-std-timing-diagram}。
...
与 PCM 标准相比，TDM PCM 标准支持更多的通道，见图 \ref{fig:i2s-tdm-pcm-std-timing-diagram} 所示。
...
\end{itemize}
I2S TX/RX 模块的串行时钟 BCK 再由 I2S\_TX/RX\_CLK 分频获得，如图 \ref{fig:i2s-clk} 所示。\hyperref[fielddesc:I2SRXCLKSEL]{I2S\_TX/RX\_CLK\_SEL} 用于选择 I2S TX/RX 的时钟源，\hyperref[fielddesc:I2SRXCLKACTIVE]{I2S\_TX/RX\_CLK\_ACTIVE} 用于使能或者关闭 I2S TX/RX 模块的时钟源。
...
在 TDM 输出模式下，I2S 支持最多 16 通道数据输出。发送通道数由 \hyperref[fielddesc:I2STXTDMTOTCHANNUM]{I2S\_TX\_TDM\_TOT\_CHAN\_NUM} 控制。例如，配置 \hyperref[fielddesc:I2STXTDMTOTCHANNUM]{I2S\_TX\_TDM\_TOT\_CHAN\_NUM} 为 5，则六个通道（通道 0 \verb+~+ 5）将用于发送数据，见图
...
\begin{itemize}
    ...
    \end{itemize}
\end{itemize}
当 I2S 处于主机 TDM 输出模式下，WS 信号由 \hyperref[fielddesc:I2STXWSIDLEPOL]{I2S\_TX\_WS\_IDLE\_POL} 和 \hyperref[fielddesc:I2STXTDMWSWIDTH]{I2S\_TX\_TDM\_WS\_WIDTH} 控制。其中，\hyperref[fielddesc:I2STXWSIDLEPOL]{I2S\_TX\_WS\_IDLE\_POL} 的值为 WS 信号的默认电平，\hyperref[fielddesc:I2STXTDMWSWIDTH]{I2S\_TX\_TDM\_WS\_WIDTH} 的值为在发送所有通道数据的过程中，WS 为默认电平的周期数。另外，\hyperref[fielddesc:I2STXHALFSAMPLEBITS]{I2S\_TX\_HALF\_SAMPLE\_BITS} 的值乘以 2，即为一个 WS 周期对应的 BCK 周期数。
...
当 I2S 处于主机 PDM 输出模式下，WS 信号的默认电平由 \hyperref[fielddesc:I2STXWSIDLEPOL]{I2S\_TX\_WS\_IDLE\_POL} 控制，WS 信号频率为 BCK 信号频率的一半。请参考 \ref{sec:i2s-tx-rx-clk} 小节配置 BCK 信号的方式配置 WS 信号，见图
...
\vspace{-1em}
    \item 配置采样频率和上采样率。\newline
    在PCM-to-PDM 输出模式下，PDM 时钟频率即为 BCK 时钟频率。采样频率 ($f_{\textrm{Sampling}}$) 与 BCK 时钟频率的关系如下：
...
在本示例中，寄存器的配置如下：
\begin{itemize}
    \item 配置 \hyperref[fielddesc:I2Ssec:i2s-tx-dataMOD]{I2S\_TX\_CHAN\_MOD} 为 2，即选择单声道模式。
...
\vspace{1em}
\begin{tiplisting}
本小节以及后续小节所述的配置，均需要通过置位 \hyperref[fielddesc:I2SRXUPDATE]{I2S\_RX\_UPDATE} 的方式来进行更新，从而将 I2S RX 寄存器数据从 APB 时钟域同步到 I2S RX 时钟域。详细配置见第 \ref{sec:i2s-configure-i2s-rx-mode} 小节。
\end{tiplisting}
\vspace{-2em}
I2S 接收数据时，从外设接口读取数据，经过通道模式控制和数据格式控制，通过 DMA 输入数据至内存。

\subsection{通道模式控制}
\chipname{} I2S 支持 TDM 和 PDM 两种输入模式。置位 \hyperref[fielddesc:I2SRXTDMEN]{I2S\_RX\_TDM\_EN} 则为 TDM 输入模式，置位 \hyperref[fielddesc:I2SRXPDMEN]{I2S\_RX\_PDM\_EN} 则为 PDM 输入模式。
...
在 TDM 输入模式下，I2S 支持最多 16 通道数据输入。接收通道数由 \hyperref[fielddesc:I2SRXTDMTOTCHANNUM]{I2S\_RX\_TDM\_TOT\_CHAN\_NUM} 控制。例如，配置 \hyperref[fielddesc:I2SRXTDMTOTCHANNUM]{I2S\_RX\_TDM\_TOT\_CHAN\_NUM} 为 5，则通道 0 \verb+~+ 5 接收数据。
...
\end{itemize}
当 I2S 处于主机 TDM 输入模式下，WS 信号由 \hyperref[fielddesc:I2SRXWSIDLEPOL]{I2S\_RX\_WS\_IDLE\_POL} 和 \hyperref[fielddesc:I2SRXTDMWSWIDTH]{I2S\_RX\_TDM\_WS\_WIDTH} 控制。其中，\hyperref[fielddesc:I2SRXWSIDLEPOL]{I2S\_RX\_WS\_IDLE\_POL} 的值为 WS 信号的默认电平，\hyperref[fielddesc:I2SRXTDMWSWIDTH]{I2S\_RX\_TDM\_WS\_WIDTH} 的值为在接收所有通道的过程中，WS 为默认电平的周期数。另外，\hyperref[fielddesc:I2SRXHALFSAMPLEBITS]{I2S\_RX\_HALF\_SAMPLE\_BITS} 的值乘以 2，即为一个 WS 周期对应的 BCK 周期数。
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{3cm}|p{3cm}|C{3.5cm}|p{3.5cm}|}
    \hline
    \rowcolor{lightgray}
...
\section{寄存器列表}\hypertarget{i2s-reg-summ}{}

请查看章节 \hyperref[glossary-access-types]{\textit{寄存器的访问类型}}，了解“访问”列缩写的含义。

\subfile{03-I2S/i2s_reg_summary_zho-CN.tex}
\section{寄存器}\label{sec:i2s-registers}
\subfile{03-I2S/i2s_reg_zho-CN.tex}
\end{document}
