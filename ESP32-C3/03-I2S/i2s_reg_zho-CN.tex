...
\label{fielddesc:I2SRXUPDATE}\item [I2S\_RX\_UPDATE] 写 1 将 I2S RX 寄存器从 APB 时钟域同步到 I2S RX 时钟域。寄存器完成更新后，该位将由硬件清除。(R/W/SC)
...
\label{fielddesc:I2SRXSTOPMODE}\item [I2S\_RX\_STOP\_MODE] 0：只有在 I2S\_RX\_START 被清除时，I2S RX 才会停止工作；1：当 I2S\_RX\_START 为 0 或 in\_suc\_eof 为 1 时，I2S RX 停止工作；2：当 I2S\_RX\_START 为 0 或 RX FIFO 为满时，I2S RX 停止工作。 (R/W)
...
\label{fielddesc:I2SRXBITORDER}\item [I2S\_RX\_BIT\_ORDER] 配置是否调换 I2S RX 待接收数据的位顺序。0：不调换。1：调换。 (R/W)
...
\label{fielddesc:I2SRXHALFSAMPLEBITS}\item [I2S\_RX\_HALF\_SAMPLE\_BITS] I2S RX 单次采样比特数的一半。I2S\_RX\_HALF\_SAMPLE\_BITS x 2 等于在一个 WS 信号中，BCK 持续的周期长。 (R/W)
\label{fielddesc:I2SRXTDMCHANBITS}\item [I2S\_RX\_TDM\_CHAN\_BITS] 在 TDM RX 模式下，每个通道的 RX 数据位等于该值 + 1。 (R/W)
\label{fielddesc:I2SRXMSBSHIFT}\item [I2S\_RX\_MSB\_SHIFT] 
控制 WS 和数据的 MSB 位之间的时序关系。1：相隔一个周期；0：上升沿对齐。(R/W)
\end{reglist}\end{regdesc}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:I2SRXCLKMDIVNUM}\item [I2S\_RX\_CLKM\_DIV\_NUM] I2S 时钟分频器的整数值。 (R/W)
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:I2SRXTDMPDMCHAN0EN}\item [I2S\_RX\_TDM\_PDM\_CHAN\regindex{n}\_EN (\regindex{n} = 0 - 7)] 1：使能 I2S TDM 或 PDM RX 通道 \regindex{n} 的有效输入数据。0：禁用该功能，在该通道中仅输入 0。 (R/W)
\label{fielddesc:I2SRXTDMCHAN8EN}\item [I2S\_RX\_TDM\_CHAN\regindex{n}\_EN (\regindex{n} = 8 - 15)] 1：使能 I2S TDM RX 通道 \regindex{n} 的有效输入数据。0：禁用该功能，在该通道中仅输入 0。 (R/W)
...
\label{fielddesc:I2Ssec:i2s-tx-dataEQUAL}\item [I2S\_TX\_CHAN\_EQUAL] 1：在 I2S TX 单声道模式或 TDM 模式下，左声道数据等于右声道数据。0：在 I2S TX 单声道模式或 TDM 通道选择模式下，I2S\_SINGLE\_DATA 为无效的通道数据。 (R/W)
...
\label{fielddesc:I2STXPCMCONF}\item [I2S\_TX\_PCM\_CONF] I2S TX 压缩/解压缩配置位。0 (atol)：A 率解压缩；1 (ltoa)：A 率压缩；2 (utol)：$\mu$ 率解压缩；3 (ltou)：u 率压缩。  (R/W)
...
\label{fielddesc:I2STXBITORDER}\item [I2S\_TX\_BIT\_ORDER] 配置是否调换 I2S TX 待发送有效数据的位顺序。0：不调换。1：调换。(R/W)
\label{fielddesc:I2STXTDMEN}\item [I2S\_TX\_TDM\_EN] 1：使能 I2S TDM TX 模式。0：禁用该模式。 (R/W)
\item [见下页]
\end{reglist}\end{regdesc}
\end{register}
...
\label{fielddesc:I2STXPDMEN}\item [I2S\_TX\_PDM\_EN] 1：使能 I2S PDM TX 模式。0：禁用该模式。 (R/W)
\label{fielddesc:I2Ssec:i2s-tx-dataMOD}\item [I2S\_TX\_CHAN\_MOD] I2S 发送单元通道配置位，更多信息见表 \ref{tab:i2s-chan-control-pdm-mode}。 (R/W)
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:I2STXCLKMDIVNUM}\item [I2S\_TX\_CLKM\_DIV\_NUM] I2S 时钟分频器的整数值。(R/W)
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:I2STXTDMCHAN0EN}\item [I2S\_TX\_TDM\_CHAN\regindex{n}\_EN (\regindex{n} = 0 - 15)] 1：使能 I2S TDM TX 通道 \regindex{n} 的有效输出数据。0：通道发送数据由 \hyperref[fielddesc:I2Ssec:i2s-tx-dataEQUAL]{I2S\_TX\_CHAN\_EQUAL} 和 \hyperref[fielddesc:I2SSINGLEDATA]{I2S\_SINGLE\_DATA} 控制，参考章节 \ref{sec:i2s-chan-control-tdm-mode}。 (R/W)
...
\vspace{-2em}
\begin{tiplisting}
上文所述的 ``a" 和 ``b" 分别为小数分频的分母部分和分子部分。更新信息，见第 \ref{sec:i2s-tx-rx-clk} 小节。
\end{tiplisting}
\end{register}
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:I2STXIDLE}\item [I2S\_TX\_IDLE] 1：I2S TX 单元 处于空闲状态。0：I2S TX 单元 处于工作状态。 (RO)
\end{reglist}\end{regdesc}
\end{register}
