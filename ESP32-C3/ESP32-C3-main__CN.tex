...
该部分介绍构成芯片系统的基本要素，包括直接内存访问 (DMA) 控制器和高性能 CPU。

\subfileinclude{33-RISCVCPU__CN}
\subfileinclude{14-DMA__CN}
...
该部分介绍芯片的系统级功能，包括系统启动、时钟、GPIO、定时器、看门狗、调试辅助、低功耗管理以及各种系统寄存器。

\subfileinclude{16-IOMUXGPIO__CN}
\subfileinclude{02-RESCLK__CN}
\subfileinclude{29-BOOTCTRL__CN}
\subfileinclude{17-INTMTRX__CN}
\subfileinclude{22-LOWPOWM__CN}
\subfileinclude{28-SYSTIMER__CN}
\subfileinclude{12-TIMG__CN}
\subfileinclude{13-WDT__CN}
\subfileinclude{31-XTWDT__CN}
\subfileinclude{08-PERMCTRL__CN}
\subfileinclude{34-WCTRL__CN}
\subfileinclude{18-SYSREG__CN}
\subfileinclude{32-DBGASSIST__CN}

\part{加密和安全组件}

该部分专门介绍芯片的安全功能，包括 SHA、AES 等加密加速器，数字签名、随机数生成、加密/解密算法等安全特性，展示了芯片在加密和安全数据处理方面的性能。

\subfileinclude{24-AES__CN}
\subfileinclude{23-HMAC__CN}
\subfileinclude{25-RSA__CN}
\subfileinclude{21-SHA__CN}
\subfileinclude{11-DIGSIG__CN}
\subfileinclude{27-EXTMEMENCR__CN}
\subfileinclude{05-RNG__CN}
\subfileinclude{30-GLITCH__CN}
...
该部分涉及系统的连接性，介绍各种带有通讯接口的模块，如 I2C、I2S、SPI、UART、USB 等，同时还包括其他用于遥控、LED 控制等功能的信号生成接口。

\subfileinclude{10-UART__CN}
\subfileinclude{19-SPI__CN}
\subfileinclude{01-I2C__CN}
\subfileinclude{03-I2S__CN}
\subfileinclude{09-USBSERIALJTAG__CN}
\subfileinclude{26-TWAI__CN}
\subfileinclude{15-LEDPWM__CN}
\subfileinclude{04-RMT__CN}
