...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:MIMPID}\item [MIMPID] 硬件实现编号。（只读）
\end{reglist}\end{regdesc}
\end{register}
...
\label{fielddesc:MSTATUSMPIE}\item [MPIE] （异常）之前的 机器中断使能。（读/写）
\label{fielddesc:MSTATUSMPP}\item [MPP] （异常）之前的机器特权模式。（读/写）
\newline 可能的值：
\begin{itemize}
...
\label{fielddesc:BRANCH_TAKEN}\item [BRANCH\_TAKEN] 采取分支跳转。（读/写）  
...
\reglabel{Reset}\regnewline%
\begin{regdesc}\begin{reglist}
\label{fielddesc:MPCCR}\item [MPCCR] 程序计数器计数的值。（读/写）
\end{reglist}\end{regdesc}
\end{register}
...
\newline 此寄存器是 \hyperref[fielddesc:GPIO_OUT]{CPU\_GPIO\_OUT} 的使能寄存器
\begin{itemize}
    \setlength\itemsep{-1em}
    \item 0：GPIO 输出关闭
    \item 1：GPIO 输出使能
