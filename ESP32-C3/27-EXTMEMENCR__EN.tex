...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{./27-EXTMEMENCR/figures/27-EXTMEMENCR-external-memory-cryptography}
...
The \begin{math}Key\end{math} is 256-bit long. The value of the \begin{math}Key\end{math} is determined by the content in one eFuse block from BLOCK4 \verb+~+ BLOCK9. For easier description, we define:
\begin{itemize}
\item Block$_A$: the block whose key purpose is EFUSE\_KEY\_PURPOSE\_XTS\_AES\_128\_KEY (please refer to Table \ref{tab:efuse-key-purpose} \textit{\nameref{tab:efuse-key-purpose}}). The 256-bit \begin{math}Key_{A}\end{math} is stored in it.
\end{itemize}

There are two possibilities of how the \begin{math}Key\end{math} is generated depending on whether Block$_A$ exists or not, as shown in Table \ref{tab:extmemencr-key-possibility}. In each case, the \begin{math}Key\end{math} can be uniquely determined by Block$_A$.

\input{./27-EXTMEMENCR/tables/27-EXTMEMENCR-key-possibility__EN}
...
\begin{itemize}
    \item Size: the \begin{math} size \end{math} of the target memory space, indicating the number of bytes encrypted in one encryption operation, which supports 16 or 32 bytes.
...
Assume a word in the target memory space is stored in \begin{math} address \end{math}, define \begin{math} offset = address \% 32 \end{math}, \textcolor{red}{\it{$n$}} =  \begin{math} \frac{offset}{4} \end{math}, then the word will be stored in register XTS\_AES\_PLAIN\_\textcolor{red}{\it{$n$}}\_REG.

The mapping between \begin{math} offset \end{math} and registers is shown in Table \ref{tab:extmemencr-data-padding}.

\input{./27-EXTMEMENCR/tables/27-EXTMEMENCR-data-padding__EN}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Set register XTS\_AES\_PHYSICAL\_ADDRESS\_REG to \begin{math}base\_addr\end{math}.
        \item Set register XTS\_AES\_LINESIZE\_REG to \begin{math}\frac{size}{32}\end{math}.
    \end{itemize}
    ...
    \item Write plaintext data to the registers block XTS\_AES\_PLAIN\_\textcolor{red}{\it{n}}\_REG (\textcolor{red}{\it{n}}: 0-7). For detailed information, please refer to Section \ref{sec:extmemncr-data-writing}. \\Please write data to registers according to your actual needs, and the unused ones could be set to arbitrary values.
    
    \item Wait for Manual Encryption block to be idle. Poll register XTS\_AES\_STATE\_REG until it reads 0 that indicates the Manual Encryption block is idle.
    
    \item Trigger manual encryption by writing 1 to register XTS\_AES\_TRIGGER\_REG.
    
    \item Wait for the encryption process completion. Poll register XTS\_AES\_STATE\_REG until it reads 2. \\ \textit{Step 1 to 5 are the steps of encrypting plaintext instructions with the Manual Encryption block using the \begin{math}Key\end{math}.}
    
    \item Write 1 to register XTS\_AES\_RELEASE\_REG to grant SPI1 the access to the encrypted ciphertext. After this, the value of register XTS\_AES\_STATE\_REG will become 3. 
    
    \item Call SPI1 to write the ciphertext in the external flash (see Chapter \ref{mod:spi} \textit{\nameref{mod:spi}}).
    
    \item Write 1 to register\\ XTS\_AES\_DESTROY\_REG to destroy the ciphertext. After this, the value of register XTS\_AES\_STATE\_REG will become 0.
...
\subfile{\modulefiles/27-EXTMEMENCR-reg-summ__EN}
...
\subfile{\modulefiles/27-EXTMEMENCR-reg__EN}
