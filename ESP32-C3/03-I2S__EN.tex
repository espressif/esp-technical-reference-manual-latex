...
\section{Features}
\begin{itemize}
    \item Supports master mode and slave mode
    \item Supports full-duplex and half-duplex communications
    \item Provides separate TX unit and RX unit, independent of each other 
    \item Supports TX unit and RX unit to work independently and simultaneously
    \item Supports a variety of audio standards: 
    \begin{itemize}
    ...
    \item Supports the following frequencies: 8 kHz, 16 kHz, 32 kHz, 44.1 kHz, 48 kHz, 88.2 kHz, 96 kHz, 128 kHz, and 192 kHz (192 kHz is not supported in 32-bit slave mode).
    \item Supports 8-/16-/24-/32-bit data communication
    \item Supports DMA access
    \item Supports standard I2S interface interrupts
\end{itemize}

\section{System Architecture}
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.9\textwidth]{03-I2S/figures/i2s_arch_20221213}}
...
\end{itemize}
I2S module supports direct access (DMA) to internal memory, see Chapter \ref{mod:dma} \textit{\nameref{mod:dma}}. 
...
\vspace{-0.5em}
\begin{tiplisting}
Any required signals of I2S must be mapped to the chip's pins via GPIO matrix, see Chapter \ref{mod:iomuxgpio} \textit{\nameref{mod:iomuxgpio}}.
\end{tiplisting}
...
The serial clock (BCK) of the I2S TX/RX unit is divided from I2S\_TX/RX\_CLK, as shown in Figure \ref{fig:i2s-clk}. \hyperref[fielddesc:I2SRXCLKSEL]{I2S\_TX/RX\_CLK\_SEL} is used to select clock source for TX/RX unit, and \hyperref[fielddesc:I2SRXCLKACTIVE]{I2S\_TX/RX\_CLK\_ACTIVE} to enable or disable the clock source.

\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.6\textwidth]{03-I2S/figures/i2s_clk_20221213}}
    \caption{I2S Clock}
    \label{fig:i2s-clk}
\end{figure}
...
The values of x, y, z, and yn1 are configured in \hyperref[fielddesc:I2STXCLKMDIVX]{I2S\_TX/RX\_CLKM\_DIV\_X}, \hyperref[fielddesc:I2STXCLKMDIVY]{I2S\_TX/RX\_CLKM\_DIV\_Y}, \hyperref[fielddesc:I2STXCLKMDIVZ]{I2S\_TX/RX\_CLKM\_DIV\_Z}, and \hyperref[fielddesc:I2STXCLKMDIVYN1]{I2S\_TX/RXCLKM\_DIV\_YN1}.
To configure the integer divider, clear \hyperref[fielddesc:I2STXCLKMDIVX]{I2S\_TX/RX\_CLKM\_DIV\_X} and \hyperref[fielddesc:I2STXCLKMDIVZ]{I2S\_TX/RX\_CLKM\_DIV\_Z}, then set \hyperref[fielddesc:I2STXCLKMDIVY]{I2S\_TX/RX\_CLKM\_DIV\_Y} to 1.
...
\begin{itemize}
    ...
    \item In I2S slave mode, make sure $f_{\textrm{I2S\_TX/RX\_CLK}}$ >= 8 * $f_{\textrm{BCK}}$. I2S module can output I2S\_MCLK\_out as the master clock for peripherals.
\end{itemize}
\end{tiplisting}
...
The \chipname{} I2S module can operate as a master or a slave in half-duplex and full-duplex communication modes, depending on the configuration of  \hyperref[fielddesc:I2SRXSLAVEMOD]{I2S\_RX\_SLAVE\_MOD} and  \hyperref[fielddesc:I2STXSLAVEMOD]{I2S\_TX\_SLAVE\_MOD}.
...
\end{itemize}
At this point, the data format control is complete. Figure  \ref{fig:i2s-tx-data-format-control} shows a complete process of TX data format control.
...
\begin{itemize}
    ...
    \item 0: the TX data to be sent by this channel is controlled by \hyperref[fielddesc:I2Ssec:i2s-tx-dataEQUAL]{I2S\_TX\_CHAN\_EQUAL}:
    \begin{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{03-I2S/figures/i2s_tdm_channel_config.png}
...
\begin{table}[H]
    ...
    \end{tabular}
\end{table}
In \textbf{normal PDM TX mode}, I2S channel mode is controlled by  \hyperref[fielddesc:I2Ssec:i2s-tx-dataMOD]{I2S\_TX\_CHAN\_MOD} and  \hyperref[fielddesc:I2STXWSIDLEPOL]{I2S\_TX\_WS\_IDLE\_POL}, see the table below. 
...
\begin{table}[H]
    ...
    \begin{threeparttable}
    \begin{tabular}{|p{2.0cm}|L{4.7cm}|L{4.7cm}|R{1.5cm}|R{1.5cm}|}
    \hline
    \rowcolor{lightgray}
...
In PDM TX aster mode, the WS level of I2S module is controlled by  \hyperref[fielddesc:I2STXWSIDLEPOL]{I2S\_TX\_WS\_IDLE\_POL}. The frequency of WS signal is half of BCK frequency. The configuration of WS signal is similar to that of BCK signal, see Section \ref{sec:i2s-tx-rx-clk} and Figure
\ref{fig:i2s-pdm-chan-control-example}.

In \textbf{PCM-to-PDM TX mode}, the PCM data from DMA is converted to PDM data and then output in PDM signal format. Configure  \hyperref[fielddesc:I2SPCM2PDMCONVEN]{I2S\_PCM2PDM\_CONV\_EN} to enable this mode. 
...
\begin{itemize}
    \item I2S\_TX\_CHAN\_MOD = 2, i.e. mono mode is selected.
    \item I2S\_TX\_WS\_IDLE\_POL = 1, i.e. both the left channel and right channel transmit the left channel data.
\end{itemize}
...
\begin{tiplisting}
Updating the configuration described in this and subsequent sections requires setting \hyperref[fielddesc:I2SRXUPDATE]{I2S\_RX\_UPDATE}, to synchronize registers from APB clock domain to RX clock domain. For more detailed configuration, see Section \ref{sec:i2s-configure-i2s-rx-mode}.
\end{tiplisting}
...
\end{itemize}
In TDM RX master mode, WS signal is controlled by \hyperref[fielddesc:I2SRXWSIDLEPOL]{I2S\_RX\_WS\_IDLE\_POL} and \hyperref[fielddesc:I2SRXTDMWSWIDTH]{I2S\_RX\_TDM\_WS\_WIDTH}.
\begin{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{3cm}|p{2.5cm}|L{3.5cm}|R{3.3cm}|}
    \hline
    \rowcolor{lightgray}
...
\begin{enumerate}
    ...
    \item Set needed TX data mode and TX channel mode as described in Section \ref{sec:i2s-tx-data}, and then set the bit \hyperref[fielddesc:I2STXUPDATE]{I2S\_TX\_UPDATE}.
...
\begin{enumerate}
    ...
    \item Set needed RX data mode and RX channel mode as described in Section \ref{sec:i2s-rx-data}, and then set the bit \hyperref[fielddesc:I2SRXUPDATE]{I2S\_RX\_UPDATE}.
...
\section{Register Summary} \hypertarget{i2s-reg-summ}{}

The abbreviations given in Column \textbf{Access} are explained in Section \hyperref[glossary-access-types]{\textit{Access Types for Registers}}.

\subfile{03-I2S/i2s_reg_summary.tex}
\section{Registers}\label{sec:i2s-registers}
\subfile{03-I2S/i2s_reg.tex}
\end{document}
